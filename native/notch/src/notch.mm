...
Napi::Value ShowHost(const Napi::CallbackInfo& info) {
  ...
  NSString *requestId = StringFromValue(presentation.Get("requestId")); Napi::Array actionsValue = presentation.Get("actions").As<Napi::Array>(); if (requestId.length == 0 || requestId.length > 128 || actionsValue.Length() > 4) return Napi::Boolean::New(info.Env(), false);
