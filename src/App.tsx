...
export default function App() {
  ...
  return (
    <AppShell active={route} onNavigate={(key) => {
      ...
      <div className={route === 'home' || route === 'tasks' || route === 'reminders' || route === 'agenda' || route === 'day' || route === 'week' || route === 'focus' || route === 'break' ? 'redesign-surface' : 'legacy-surface'} onClickCapture={(event) => { const button = (event.target as HTMLElement).closest('button'); if (route === 'tasks' && button?.textContent?.trim() === '+ New task') { event.preventDefault(); event.stopPropagation(); setTaskCreateOpen(true); } if (route === 'reminders' && button?.textContent?.trim() === '+ New reminder') { event.preventDefault(); event.stopPropagation(); setReminderCreateOpen(true); } }}>{content}</div>
