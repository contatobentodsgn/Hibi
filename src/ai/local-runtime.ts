...
type Hooks = Readonly<{ onDataChanged?: () => void; onTaskCompleted?: (title: string, id: string) => void; onFocusStarted?: () => void; onAudit?: (event: AiAuditEvent) => void; onUsage?: (event: AiRuntimeUsageEvent) => void; integrations?: IntegrationActionBridge }>;
...
export function createLocalToolRegistry(repository: LocalRepository, hooks: Hooks = {}): ToolRegistry {
  ...
  // Só avisa na transição para concluída: editar uma tarefa já concluída não é uma nova conclusão.
  register({ name: 'task.update', description: 'Update a local task title, duration, or status.', risk: 'reversible', externallyVisible: true, inputSchema: { type: 'object', required: ['id'] }, validate: (args) => Boolean(id(args)) && (args.title === undefined || isText(args.title)) && (args.durationMinutes === undefined || Number.isInteger(args.durationMinutes) && Number(args.durationMinutes) >= 5 && Number(args.durationMinutes) <= 480) && entityStatus(args.status), execute: (args) => { const wasCompleted = repository.getTask(id(args))?.status === 'completed'; const task = repository.updateTask(id(args), { ...(args.title === undefined ? {} : { title: title(args) }), ...(args.durationMinutes === undefined ? {} : { durationMinutes: Number(args.durationMinutes) }), ...(args.status === undefined ? {} : { status: args.status as 'open' | 'completed' | 'paused' }) }); hooks.onDataChanged?.(); if (task.status === 'completed' && !wasCompleted) hooks.onTaskCompleted?.(task.title, task.id); return { summary: `Tarefa atualizada: ${task.title}`, data: { id: task.id } }; } });
