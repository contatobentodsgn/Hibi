...
export function SettingsView({ data, onEvent, onReset, onRestore, onTestNotification, aiFallbackPolicy, onAiFallbackPolicyChange, aiUsage, onApplyImport, onApplyNotion, focusSettings, onFocusSettingsChange }: Props) {
  ...
  useEffect(() => { if (tab !== 'Data') return; let active = true; setRestorePointsNotice(''); void workspaceStore.restorePoints().then((points) => { if (active) setRestorePoints(points); }).catch(() => { if (active) { setRestorePoints([]); setRestorePointsNotice('Restore points are available in the Hibi desktop app.'); } }); return () => { active = false; }; }, [tab, workspaceStore]);
  const restorePointText = (point: WorkspaceRestorePoint) => { const key = restorePointLabelKey(point.label); return key ? t(key) : point.label; };
  const restorePointDate = (value: string) => { try { return new Intl.DateTimeFormat(language === 'pt' ? 'pt-BR' : 'en-US', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value)); } catch { return value; } };
  const restorePointSize = (bytes: number) => bytes < 1024 ? `${bytes} B` : bytes < 1024 * 1024 ? `${Math.round(bytes / 1024)} KB` : `${(bytes / (1024 * 1024)).toFixed(1)} MB`;
  const restoreFromPoint = async (point: WorkspaceRestorePoint) => { if (!window.confirm(`Restore workspace from “${restorePointText(point)}”? The current workspace will be replaced.`)) return; try { await workspaceStore.restore(point.id); onEvent('restore', restorePointText(point), 'pass'); window.location.reload(); } catch (error) { setRestorePointsNotice(error instanceof Error ? error.message : 'Could not restore this point.'); onEvent('restore', restorePointText(point), 'fail'); } };
  const tabs = ['General', 'AI', 'Integrations', 'Focus', 'Notifications', 'Data', 'About'] as const;
  return <div className="view settings-view"><div className="view-heading"><div><p className="eyebrow">HIBI STUDY REPLICA</p><h1>Settings</h1><p className="muted">Local preferences · {data.tasks.length} tasks · {data.reminders.length} reminders</p></div></div><div className="settings-layout"><aside className="settings-tabs" aria-label="Settings sections">{tabs.map((item) => <button key={item} className={tab === item ? 'active' : ''} aria-current={tab === item ? 'page' : undefined} onClick={() => { setTab(item); onEvent('navigation', `Settings · ${item}`); }}>{item}</button>)}</aside><section className="settings-card"><h2 className="settings-section-title">{tab}</h2>{tab === 'General' && <><Setting title="Language" detail="Interface language"><select aria-label="Language" value={language} onChange={(event) => { const next = event.target.value as Locale; setLanguage(next); onEvent('edit', 'Language', next); }}><option value="pt">Português (Brasil)</option><option value="en">English</option></select></Setting><Setting title={t('settings.theme.title')} detail={t('settings.theme.detail')}><select aria-label={t('settings.theme.title')} value={themePreference} onChange={(event) => { const next = event.target.value; if (isThemePreference(next)) { setThemePreference(next); onEvent('edit', 'Theme', next); } }}><option value="system">{t('settings.theme.system')}</option><option value="light">{t('settings.theme.light')}</option><option value="dark">{t('settings.theme.dark')}</option></select></Setting><Setting title="Time format" detail="Use clear, exact times across calendar and reminders"><button className={`toggle ${twentyFourHour ? 'on' : ''}`} aria-pressed={twentyFourHour} onClick={() => { const next = !twentyFourHour; setTwentyFourHour(next); onEvent('edit', 'Time format', next ? '24h' : '12h'); }}><span />{twentyFourHour ? '24-hour' : 'AM / PM'}</button></Setting><Setting title="Launch at login" detail="Open Hibi automatically when this Mac starts"><button className={`toggle ${launchAtLogin ? 'on' : 'off'}`} aria-pressed={launchAtLogin} onClick={async () => { const bridge = window.hibiDesktop; if (!bridge?.setOpenAtLogin || !bridge.getOpenAtLogin) { setLaunchNotice('Available in the desktop app.'); return; } try { const applied = await syncLaunchAtLogin({ setOpenAtLogin: bridge.setOpenAtLogin, getOpenAtLogin: bridge.getOpenAtLogin }, !launchAtLogin); setLaunchAtLogin(applied); setLaunchNotice(applied === !launchAtLogin ? '' : 'macOS kept its current startup setting.'); onEvent('edit', 'Launch at login', String(applied)); } catch { setLaunchNotice('Could not update the macOS startup setting.'); } }}><span />{launchAtLogin ? 'On' : 'Off'}</button>{launchNotice && <p className="muted" aria-live="polite" style={{ margin: '8px 0 0' }}>{launchNotice}</p>}</Setting><NotchDisplaySettings onEvent={onEvent} /></>}{tab === 'AI' && <AiSettings onEvent={onEvent} fallbackPolicy={aiFallbackPolicy} onFallbackPolicyChange={onAiFallbackPolicyChange} usage={aiUsage} />}{tab === 'Integrations' && <IntegrationsView onEvent={onEvent} localRecords={data.tasks.map((task) => ({ id: task.id, title: task.title, remoteRef: task.remoteRef }))} localTasks={data.tasks} localBlocks={data.blocks} onApplyImport={onApplyImport} onApplyNotion={onApplyNotion} />}{tab === 'Focus' && <FocusSettingsPanel settings={focus} onChange={onFocusSettingsChange} data={data} onEvent={onEvent} />}{tab === 'Notifications' && <><Setting title="Native notifications" detail="Verify macOS alerts for reminders and deadlines"><button className="outline" onClick={testNotification}>Send test notification</button></Setting><div className="settings-divider" /><div className="integration-status"><Status name="macOS notifications" state="Connected" /><Status name="Scheduled reminders" state={`${data.reminders.filter((item) => item.status !== 'paused').length} active`} /></div></>}{tab === 'Data' && <><Setting title="Workspace backup" detail="Export or restore tasks, calendar, reminders, notes, habits, goals and safe preferences"><div style={{ display: 'flex', gap: 8 }}><button className="outline" onClick={exportWorkspace}>Export backup</button><button className="outline" onClick={() => importRef.current?.click()} disabled={!onRestore}>Restore backup</button><input ref={importRef} aria-label="Choose Hibi workspace backup" type="file" accept="application/json,.json" onChange={(event) => void importWorkspace(event)} style={{ display: 'none' }} /></div></Setting>{backupNotice && <p className="muted" role="status" aria-live="polite" style={{ margin: '0 0 16px' }}>{backupNotice}</p>}<Setting title="Restore points" detail="Return the workspace to a safe point created before a destructive action"><div className="restore-points" aria-live="polite">{restorePointsNotice ? <p className="muted" role="status">{restorePointsNotice}</p> : restorePoints.length ? <ul className="audit-history" aria-label="Workspace restore points">{restorePoints.map((point) => <li className="event-row" key={point.id}><div><strong>{restorePointText(point)}</strong><span>{restorePointDate(point.createdAt)} · {restorePointSize(point.bytes)}</span></div><button className="outline" onClick={() => void restoreFromPoint(point)}>Restore</button></li>)}</ul> : <p className="muted">No restore points available. Open Hibi in the desktop app to access them.</p>}</div></Setting><Setting title="Support bundle" detail="Export a local diagnostic snapshot for review"><button className="outline" onClick={exportBundle}>Export JSON</button></Setting><Setting title="Study data" detail={`Restore the local seed dataset · ${data.blocks.length} schedule blocks`}><button className="outline" onClick={onReset}>Reset study data</button></Setting></>}{tab === 'About' && <><p className="muted">Hibi is an offline-first personal planning workspace.</p><div className="settings-divider" /><h3 className="eyebrow">INTEGRATION STATUS</h3><div className="integration-status"><Status name="Local calendar" state="Connected" /><Status name="ICS import/export" state="Available locally" /><Status name="External calendar sync" state="Not configured" /><Status name="Brain / hardware" state="Unavailable offline" /></div></>}</section></div></div>;
}
