...
import {
  ...
  type WorkspaceRestorePoint,
} from "../data/workspace-store";
import {
  restorePointDate,
  restorePointSize,
  restorePointText,
  restorePointsEmptyKey,
} from "./restore-point-format";
...
export type SettingsViewProps = {
  ...
  onReset: () => void;
  onRestore?: (data: StudyData, preferences: WorkspacePreferences) => void;
  onTestNotification?: () => Promise<boolean>;
  aiFallbackPolicy?: AiFallbackPolicy;
  onAiFallbackPolicyChange?: (policy: AiFallbackPolicy) => void;
  aiUsage?: readonly AiUsageRecord[];
  onApplyImport?: (
    candidate: import("../integrations/imports").ImportCandidate,
    decision: import("../integrations/imports").ImportDecision,
    localId?: string,
    connectorId?: string,
  ) => void;
  onApplyNotion?: (
    mutations: readonly NotionLocalMutation[],
  ) => readonly import("../domain/models").Task[];
...
export function aiSaveFailureMessage(
  error: unknown,
  ...
  if (message.includes("invalid_request"))
    return fillTemplate(t("settings.ai.invalidRequest"), {
      endpoint: config.endpoint,
      model: config.model,
    });
  if (message.includes("invalid_credentials"))
    return t("settings.ai.invalidCredentials");
  if (message.includes("invalid_response"))
    return t("settings.ai.invalidResponse");
...
export function usageSummaryFor(entries: readonly AiUsageRecord[]) {
  return entries.reduce(
    (summary, entry) => ({
      ...
      estimatedCost: summary.estimatedCost + (entry.estimatedCost ?? 0),
    }),
    { turns: 0, totalTokens: 0, estimatedCost: 0 },
  );
}

export async function syncLaunchAtLogin(
...
export function AiSettings({
  ...
  usage = [],
}: Pick<Props, "onEvent" | "aiUsage"> & {
  fallbackPolicy?: AiFallbackPolicy;
  onFallbackPolicyChange?: (policy: AiFallbackPolicy) => void;
  usage?: readonly AiUsageRecord[];
}) {
  const t = useT();
    ...
    void window.pixanoDesktop
      ...
      .then((saved) => {
        if (active) {
          ...
          setNotice(
            saved.hasApiKey
              ? "A key is securely stored in Keychain."
              : "No API key is stored.",
          );
        }
      })
      .catch(() => {
        if (active) setNotice("AI settings are available in the desktop app.");
      });
    return () => {
      active = false;
    };
  ...
  const save = async () => {
    try {
      ...
      if (saved) {
        ...
        setNotice(
          saved.hasApiKey
            ? "AI settings saved. The key remains only in Keychain."
            : "AI settings saved.",
        );
        onEvent("edit", `AI provider: ${saved.provider}`, "pass");
  ...
  const removeKey = async () => {
    try {
      ...
      if (saved) {
        ...
        onEvent("edit", "Removed AI API key", "pass");
      }
    } catch (error) {
      setNotice(
  ...
  return (
    <div className="ai-settings">
      <Setting title="Provider" detail="Choose where AI requests are sent">
        <select
          aria-label="Provider"
      ...
      <Setting
        title="Endpoint"
        detail="HTTPS endpoint for the compatible provider"
      >
        <input
          aria-label="Endpoint"
          disabled={!external}
      ...
      <Setting
        title="Model preset"
        detail={
          recommendedModelPresets.find((preset) => preset.id === modelPreset)
            ?.description ?? ""
        }
      >
        <select
          ...
          value={modelPreset}
          onChange={(event) =>
            setModelPreset(event.target.value as AiModelPresetId)
          }
        >
          {recommendedModelPresets.map((preset) => (
      ...
      <Setting
        title="Model"
        detail="Enter the model identifier supported by your provider"
      >
        <input
          aria-label="Model"
          disabled={!external}
      ...
      <Setting
        ...
        detail="Choose what happens when a configured provider is temporarily unavailable"
      >
        <select
          aria-label="Fallback policy"
          value={fallbackPolicy}
          onChange={(event) => {
            const policy = event.target.value as AiFallbackPolicy;
            if (
              policy === "ask" ||
              policy === "automatic" ||
              policy === "never"
            ) {
              onFallbackPolicyChange?.(policy);
              onEvent("edit", `AI fallback: ${policy}`, "pass");
            }
          }}
        >
          <option value="ask">Ask before using local fallback</option>
          <option value="automatic">Automatically use local fallback</option>
          <option value="never">Never use local fallback</option>
        </select>
      </Setting>
      <Setting title="API key" detail="Stored only in macOS Keychain">
        <div style={{ display: "flex", gap: 8 }}>
          <input
            ...
            autoComplete="off"
            placeholder={
              config.hasApiKey ? "Key stored securely" : "Paste a key to save"
            }
            onChange={(event) => setApiKey(event.target.value)}
          />
          {config.hasApiKey && (
            <button className="outline" onClick={() => void removeKey()}>
              Remove key
            </button>
          )}
        </div>
      ...
      <Setting
        title="Local usage"
        detail={`${usageSummary.turns} completed turns · ${usageSummary.totalTokens} tokens · ${usageSummary.estimatedCost ? `estimated $${usageSummary.estimatedCost.toFixed(4)}` : "cost unavailable for unknown models"}`}
      >
        <span className="setting-value">Local only</span>
      </Setting>
      <p className="muted">
        The endpoint, key, and model are tested before any setting is saved.
      </p>
      <p className="muted" aria-live="polite" style={{ margin: "12px 0" }}>
        {notice}
      </p>
      <button className="primary" onClick={() => void save()}>
        {external ? "Test connection & save" : "Save AI settings"}
      </button>
    </div>
  );
}
...
export function FocusSettingsPanel({
  ...
  const minutes = (count: number) =>
    pluralize(
      language,
      count,
      "focus.count.minute.one",
      "focus.count.minute.other",
    );
  ...
  const alertsPerDay = previewDailyAlerts(
    buildNotificationEntries(data),
    settings,
  );
  ...
  const duration = (totalSeconds: number) =>
    totalSeconds < 60
      ? pluralize(
          language,
          totalSeconds,
          "focus.count.second.one",
          "focus.count.second.other",
        )
  ...
  return (
    <>
      <Setting
        title={t("focus.settings.session.title")}
        detail={t("focus.settings.session.detail")}
      >
        <select
          aria-label={t("focus.settings.session.title")}
          value={settings.sessionMinutes}
          onChange={(event) =>
            update(
              { sessionMinutes: Number(event.target.value) },
              `session ${event.target.value}m`,
            )
          }
        >
      ...
      <Setting
        title={t("focus.settings.activeHours.title")}
        detail={t("focus.settings.activeHours.detail")}
      >
        <div style={{ display: "flex", gap: 8 }}>
          <input
            aria-label={t("focus.settings.activeHours.start")}
            type="time"
            value={settings.activeStart}
            onChange={(event) =>
              update(
                { activeStart: event.target.value },
                `active from ${event.target.value}`,
              )
            }
          />
          <input
            aria-label={t("focus.settings.activeHours.end")}
            type="time"
            value={settings.activeEnd}
            onChange={(event) =>
              update(
                { activeEnd: event.target.value },
                `active until ${event.target.value}`,
              )
            }
          />
        </div>
      ...
      <Setting
        title={t("focus.settings.preset.title")}
        detail={t("focus.settings.preset.detail")}
      >
        <select
          aria-label={t("focus.settings.preset.title")}
          value={settings.nudgePreset}
          onChange={(event) =>
            update(
              { nudgePreset: event.target.value as NudgePreset },
              `preset ${event.target.value}`,
            )
          }
        >
          {NUDGE_PRESET_IDS.map((preset) => (
            <option
              key={preset}
              value={preset}
            >{`${t(`focus.settings.preset.${preset}` as DictionaryKey)} · ${minutes(NUDGE_PRESETS[preset])}`}</option>
          ))}
        </select>
      </Setting>
      <Setting
        title={t("focus.settings.preview.title")}
        detail={t("focus.settings.preview.detail")}
      >
        <span className="setting-value">
          {pluralize(
            language,
            alertsPerDay,
            "focus.count.alert.one",
            "focus.count.alert.other",
          )}
        </span>
      </Setting>
      <Setting
        title={t("focus.settings.away.title")}
        detail={t("focus.settings.away.detail")}
      >
        <select
          aria-label={t("focus.settings.away.title")}
          value={settings.awayBehavior}
          onChange={(event) =>
            update(
              { awayBehavior: event.target.value as AwayBehavior },
              `away ${event.target.value}`,
            )
          }
        >
          {AWAY_BEHAVIORS.map((behavior) => (
            <option key={behavior} value={behavior}>
              {t(`focus.settings.away.${behavior}` as DictionaryKey)}
            </option>
          ))}
        </select>
      </Setting>
      <Setting
        title={t("focus.settings.idle.title")}
        detail={
          watchesPresence
            ? t("focus.settings.idle.detail")
            : t("focus.settings.idle.detailKeep")
        }
      >
        <select
          aria-label={t("focus.settings.idle.title")}
          disabled={!watchesPresence}
          value={settings.idleMinutes}
          onChange={(event) =>
            update(
              { idleMinutes: Number(event.target.value) },
              `idle ${event.target.value}m`,
            )
          }
        >
          {IDLE_MINUTES.map((value) => (
            <option key={value} value={value}>
              {withDuration("focus.settings.idle.option", value * 60)}
            </option>
          ))}
        </select>
      </Setting>
      <Setting
        title={t("focus.settings.loop.title")}
        detail={t("focus.settings.loop.detail")}
      >
        <select
          aria-label={t("focus.settings.loop.title")}
          value={settings.focusLoopAnimation}
          onChange={(event) =>
            update(
              { focusLoopAnimation: event.target.value as FocusLoopAnimation },
              `loop ${event.target.value}`,
            )
          }
        >
          {FOCUS_LOOP_ANIMATIONS.map((loop) => (
            <option key={loop} value={loop}>
              {t(`focus.settings.loop.${loop}` as DictionaryKey)}
            </option>
          ))}
        </select>
      </Setting>
      <Setting
        title={t("focus.settings.screen.title")}
        detail={t("focus.settings.screen.detail")}
      >
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <select
            aria-label={t("focus.settings.screen.title")}
            value={settings.screenTimeoutSeconds}
            onChange={(event) =>
              update(
          ...
          >
            {SCREEN_TIMEOUT_SECONDS.map((value) => (
              <option key={value} value={value}>
                {withDuration("focus.settings.screen.option", value)}
              </option>
            ))}
          </select>
          {!assistantConnected && (
...
export function SettingsWorkspace({
  ...
  const { language, setLanguage, twentyFourHour, setTwentyFourHour } =
    useLocale();
  const t = useT();
  ...
  const [restorePoints, setRestorePoints] = useState<
    readonly WorkspaceRestorePoint[]
  >([]);
  ...
  const exportWorkspace = () => {
    const backup = createWorkspaceBackup(data, workspacePreferences());
    ...
    setBackupNotice(
      "Workspace backup exported. API keys and Keychain items are never included.",
    );
    onEvent("export", "Exported workspace backup", "pass");
  };
  const importWorkspace = async (
    ...
    try {
      const backup = parseWorkspaceBackup(await file.text(), data);
      if (
        !window.confirm(
          "Replace this local workspace with the imported backup? This cannot be undone unless you export a backup first.",
        )
      )
        return;
      onRestore(backup.data, backup.preferences);
      setLanguage(backup.preferences.language);
      setTwentyFourHour(backup.preferences.twentyFourHour);
      onFocusSettingsChange?.(backup.preferences.focus);
      setBackupNotice(`Workspace restored from ${file.name}.`);
      onEvent("import", "Restored workspace backup", "pass");
    } catch (error) {
      setBackupNotice(
      ...
      onEvent("import", "Rejected workspace backup", "fail");
    }
  };
  useEffect(() => {
    if (tab !== "Data") return;
    let active = true;
    setRestorePointsNotice("");
    void workspaceStore
      .restorePoints()
      .then((points) => {
        if (active) setRestorePoints(points);
      })
      .catch((error) => {
        if (active) {
          setRestorePoints([]);
          setRestorePointsNotice(
            error instanceof Error
              ? error.message
              : "Could not read the restore points.",
          );
        }
      });
    return () => {
  ...
  const restoreFromPoint = async (point: WorkspaceRestorePoint) => {
    const name = restorePointText(point, t);
    if (
      !window.confirm(
        `Restore workspace from “${name}”? The current workspace will be replaced.`,
      )
    )
      return;
    try {
      await workspaceStore.restore(point.id);
      onEvent("restore", name, "pass");
      window.location.reload();
    } catch (error) {
      setRestorePointsNotice(
        error instanceof Error
          ? error.message
          : "Could not restore this point.",
      );
      onEvent("restore", name, "fail");
    }
  };
      ...
      <div className="settings-layout">
        ...
        <section className="settings-card">
            ...
            <>
              ...
              >
                <button
                  ...
                  aria-pressed={twentyFourHour}
                  onClick={() => {
                    const next = !twentyFourHour;
                    setTwentyFourHour(next);
                    onEvent("edit", "Time format", next ? "24h" : "12h");
                  }}
                >
                  <span />
              ...
              >
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const applied = await syncLaunchAtLogin(
                        {
                          setOpenAtLogin: bridge.setOpenAtLogin,
                          getOpenAtLogin: bridge.getOpenAtLogin,
                        },
                        !launchAtLogin,
                      );
          ...
          {(tab === "AI" || tab === "Assistant") && (
            <>
              <AiSettings
                onEvent={onEvent}
                fallbackPolicy={aiFallbackPolicy}
                onFallbackPolicyChange={onAiFallbackPolicyChange}
                usage={aiUsage}
              />
              <div className="settings-divider" />
              <LocalModelSettings onEvent={onEvent} />
            </>
          )}
          {tab === "Integrations" && (
            <IntegrationsWorkspace
              onEvent={onEvent}
              localRecords={data.tasks.map((task) => ({
                id: task.id,
                title: task.title,
          ...
          {tab === "Focus" && (
            <FocusSettingsPanel
              settings={focus}
              onChange={onFocusSettingsChange}
              data={data}
              onEvent={onEvent}
            />
          )}
          {tab === "Notifications" && (
            <>
              <Setting
                ...
                detail="Verify macOS alerts for reminders and deadlines"
              >
                <button className="outline" onClick={testNotification}>
                  Send test notification
                </button>
              </Setting>
              <div className="settings-divider" />
              <div className="integration-status">
                <Status name="macOS notifications" state="Connected" />
                <Status
                  name="Scheduled reminders"
                  state={`${data.reminders.filter((item) => item.status !== "paused").length} active`}
                />
              </div>
            </>
          )}
          {tab === "Data" && (
            <>
              <Setting
                ...
                detail="Export or restore tasks, calendar, reminders, notes, habits, goals and safe preferences"
              >
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="outline" onClick={exportWorkspace}>
                    Export backup
                  </button>
                  <button
                    className="outline"
              ...
              <Setting
                ...
                detail="Return the workspace to a safe point created before a destructive action"
              >
                <div className="restore-points" aria-live="polite">
                  {restorePointsNotice && (
                  ...
                  {restorePoints.length ? (
                    <ul
                      ...
                      aria-label="Workspace restore points"
                    >
                      {restorePoints.map((point) => (
                        <li className="event-row" key={point.id}>
                          <div>
                            <strong>{restorePointText(point, t)}</strong>
                            <span>
                              {restorePointDate(point.createdAt, language)} ·{" "}
                              {restorePointSize(point.bytes)}
                            </span>
                          </div>
                          <button
                            className="outline"
                            onClick={() => void restoreFromPoint(point)}
                          >
                            Restore
                          </button>
                        </li>
                    ...
                    <p className="muted">
                      {t(restorePointsEmptyKey(desktopBridge))}
                    </p>
                  )}
                </div>
              </Setting>
...
function Status({ name, state }: { name: string; state: string }) {
  return (
