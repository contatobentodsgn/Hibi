...
function dateLabel(date: string, locale: 'pt' | 'en') { return localNoon(date).toLocaleDateString(locale === 'pt' ? 'pt-BR' : 'en-US', { weekday: 'long', day: 'numeric', month: 'long' }); }
function hourTime(hour: number) { return `${String(hour).padStart(2, '0')}:00`; }
function periodLabel(mode: AgendaDisplayMode, date: string, locale: 'pt' | 'en') { if (mode === 'day') return dateLabel(date, locale); const days = weekDays(date); const connector = locale === 'pt' ? ' de ' : ' – '; return `${days[0].slice(8, 10)}–${days[6].slice(8, 10)}${connector}${localNoon(days[0]).toLocaleDateString(locale === 'pt' ? 'pt-BR' : 'en-US', { month: 'long' })}`; }
...
export function AgendaScreen({ data, mode, date: initialDate, onModeChange, onDateChange, onEvent, onCreateBlock, onDeleteBlock, onMoveBlock }: Props) {
  ...
  const dayNames = ['agenda.weekday.sun', 'agenda.weekday.mon', 'agenda.weekday.tue', 'agenda.weekday.wed', 'agenda.weekday.thu', 'agenda.weekday.fri', 'agenda.weekday.sat'] as const;
  const categoryName = (category: ScheduleBlock['category']) => t(`agenda.category.${category}` as const);
  ...
  return <HibiUiRoot className="agenda-screen">
    <SectionHeader title={t('agenda.title')} subtitle={periodLabel(mode, date, language)} actions={<Button variant="primary" onPress={() => createAt(date, 9)}><Plus size={17} />{t('agenda.createBlock')}</Button>} />
    <div className="agenda-screen__toolbar"><Button isIconOnly variant="ghost" aria-label={t(mode === 'day' ? 'agenda.previousDay' : 'agenda.previousWeek')} onPress={() => movePeriod(-1)}><ChevronLeft size={18} /></Button><Button variant="secondary" onPress={() => { const today = todayKey(); setDate(today); onDateChange?.(today); onEvent('navigation', `Agenda · ${t('agenda.today')}`); }}>{t('agenda.today')}</Button><Button isIconOnly variant="ghost" aria-label={t(mode === 'day' ? 'agenda.nextDay' : 'agenda.nextWeek')} onPress={() => movePeriod(1)}><ChevronRight size={18} /></Button><div className="agenda-screen__modes" role="tablist" aria-label={t('agenda.mode')}><button type="button" role="tab" aria-selected={mode === 'day'} onClick={() => onModeChange('day')}>{t('agenda.day')}</button><button type="button" role="tab" aria-selected={mode === 'week'} onClick={() => onModeChange('week')}>{t('agenda.week')}</button></div><div className="agenda-screen__layers" aria-label={t('agenda.filters')}>{([['schedule', t('agenda.title')], ['important', t('agenda.important')], ['wellbeing', t('agenda.wellbeing')] as const]).map(([value, label]) => <button key={value} type="button" aria-pressed={layer === value} onClick={() => { setLayer(value as AgendaLayer); onEvent('filter', `Agenda · ${label}`); }}>{label}</button>)}</div>{mode === 'day' && <><Button variant="secondary" onPress={() => setShowAllHours((value) => !value)}>{t('agenda.allHours')}</Button><Button variant="secondary" onPress={() => setShowConflicts(true)}>{t('agenda.checkPlan')}</Button></>}<label className="agenda-screen__ics-import"><span>{t('agenda.import')}</span><input hidden type="file" accept=".ics,text/calendar" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importIcs(file); }} /></label><Button variant="secondary" onPress={exportIcs}>{t('agenda.export')}</Button></div>
    ...
    <div className="agenda-screen__body"><Card className="agenda-screen__grid-card"><div className={`agenda-screen__grid agenda-screen__grid--${mode}`}><div className="agenda-screen__corner"><Clock3 size={14} /></div>{visibleDays.map((day) => <div className="agenda-screen__day-head" key={day}><span>{t(dayNames[localNoon(day).getDay()])}</span><strong>{day.slice(8, 10)}</strong></div>)}{hours.map((hour) => <div className="agenda-screen__row" key={hour}><time>{hourTime(hour)}</time>{visibleDays.map((day) => { const hourBlocks = blocks.filter((block) => toDateKey(block.start) === day && Number(block.start.slice(11, 13)) === hour); return <div className="agenda-screen__slot" key={`${day}-${hour}`} role="button" tabIndex={0} aria-label={t('agenda.addBlock').replace('{day}', day).replace('{time}', hourTime(hour))} onClick={() => createAt(day, hour)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); createAt(day, hour); } }}><span className="agenda-screen__slot-create" aria-hidden="true">+</span>{hourBlocks.map((block) => <button className="agenda-screen__event" data-category={block.category} key={block.id} onClick={(event) => { event.stopPropagation(); onDeleteBlock?.(block.id); }} aria-label={t('agenda.deleteBlock').replace('{title}', block.title).replace('{time}', block.start.slice(11, 16)).replace('{day}', day)}><strong>{block.title}</strong><span>{block.start.slice(11, 16)} · {durationMinutes(block)} min</span><HibiTag tone={categoryTone[block.category]}>{categoryName(block.category)}</HibiTag></button>)}</div>; })}</div>)}</div>{blocks.length === 0 && <HibiEmptyState icon={CalendarDays} tone="lavender" title={t('agenda.emptyTitle')} description={t('agenda.emptyDetail')} action={<Button variant="secondary" onPress={() => createAt(date, 9)}>{t('agenda.createBlock')}</Button>} />}</Card>
      <aside className="agenda-screen__detail" aria-label={t('agenda.detail')}>{selected ? <BlockDetails block={selected} onClose={() => setSelectedId(null)} onDelete={() => { onDeleteBlock?.(selected.id); setSelectedId(null); }} onSave={editSelected} /> : <Card><CalendarDays size={20} /><h2>{t('agenda.timeOverview')}</h2><p>{t('agenda.timeOverviewDetail')}</p></Card>}</aside>
    </div>
    ...
    {pendingCalendarAction && <Card className="agenda-screen__calendar-confirmation" role="alert"><div><strong>{t('agenda.externalConfirm')}</strong><p>{t('agenda.externalDescription').replace('{summary}', pendingCalendarAction.summary)}</p></div><div><Button variant="secondary" onPress={() => { setPendingCalendarAction(null); setCalendarNotice(t('agenda.cancel')); }}>{t('agenda.cancel')}</Button><Button variant="primary" onPress={() => void confirmCalendarAction()}>{t('agenda.confirm')}</Button></div></Card>}
...
function BlockDetails({ block, onClose, onDelete, onSave }: Readonly<{ block: ScheduleBlock; onClose: () => void; onDelete: () => void; onSave: (start: string, end: string) => void }>) {
  ...
  return <Card><div className="agenda-screen__detail-title"><HibiTag tone={categoryTone[block.category]}>{t(categoryKey)}</HibiTag><Button isIconOnly variant="ghost" aria-label={t('agenda.closeDetail')} onPress={onClose}>×</Button></div><h2>{block.title}</h2><p>{dateLabel(block.start.slice(0, 10), language)}</p><form onSubmit={(event) => { event.preventDefault(); onSave(`${start}:00`, `${end}:00`); }}><label>{t('agenda.start')}<input type="datetime-local" value={start} onChange={(event) => setStart(event.target.value)} /></label><label>{t('agenda.end')}<input type="datetime-local" value={end} onChange={(event) => setEnd(event.target.value)} /></label><Button type="submit" variant="primary">{t('agenda.saveTime')}</Button></form><Button variant="danger" onPress={onDelete}>{t('agenda.delete')}</Button></Card>;
}
