...
function scheduleText(reminder: Reminder, t?: Translator): string {
  ...
  return `${recurrence.frequency === 'daily' ? t?.('reminders.daily') ?? 'Todos os dias' : t?.('reminders.weekly') ?? 'Toda semana'} · ${time}`;
}
...
export function RemindersScreen({ data, onEvent, onReminderStatusChange, onCreateReminder, onRenameReminder, onDeleteReminder, onEditReminderSchedule }: Props) {
  ...
  return <HibiUiRoot className="reminders-screen">
    ...
    <div className="reminders-screen__layout">
      ...
      <Card className="reminders-screen__summary"><h2>Seu próximo cuidado.</h2>{rhythm.next ? <><strong>{rhythm.next.title}</strong><p>{scheduleText(rhythm.next, t)}</p></> : <p>Nenhum lembrete ativo agora.</p>}<div><span>{rhythm.overdue}<small>atrasados</small></span><span>{rhythm.paused}<small>pausados</small></span></div></Card>
    </div>
    {editing && <ActionDialog trigger={<Button className="reminders-screen__hidden-trigger">Editar lembrete</Button>} isOpen onOpenChange={(open) => !open && setEditing(null)} title="Editar lembrete" description="Confira o próximo horário antes de salvar."><form className="reminders-screen__form" onSubmit={(event) => { event.preventDefault(); saveEdit(); }}><label>Nome<input value={titleDraft} onChange={(event) => setTitleDraft(event.target.value)} autoFocus required /></label><label>Repetição<select value={scheduleDraft.frequency} onChange={(event) => { setScheduleDraft((draft) => ({ ...draft, frequency: event.target.value as EditedReminderSchedule['frequency'] })); setPastError(false); }}><option value="one-time">Uma vez</option><option value="daily">{t('reminders.daily')}</option><option value="weekly">{t('reminders.weekly')}</option></select></label>{scheduleDraft.frequency === 'one-time' && <label>Data<input type="date" value={scheduleDraft.date} onChange={(event) => { setScheduleDraft((draft) => ({ ...draft, date: event.target.value })); setPastError(false); }} required /></label>}<label>Horário<input type="time" value={scheduleDraft.time} onChange={(event) => { setScheduleDraft((draft) => ({ ...draft, time: event.target.value })); setPastError(false); }} required /></label>{scheduleDraft.frequency === 'weekly' && <fieldset><legend>Em quais dias?</legend><div>{weekdays.map(([label, day]) => <label key={day} className="reminders-screen__weekday"><input type="checkbox" checked={scheduleDraft.weekdays.includes(day)} onChange={() => setScheduleDraft((draft) => ({ ...draft, weekdays: draft.weekdays.includes(day) ? draft.weekdays.filter((item) => item !== day) : [...draft.weekdays, day].sort() }))} />{label}</label>)}</div></fieldset>}<p className="reminders-screen__preview">Próxima ocorrência: <strong>{draftScheduleText(scheduleDraft, t)}</strong></p>{pastError && <p role="alert" className="reminders-screen__error">Escolha um horário futuro para este lembrete.</p>}<div className="reminders-screen__confirm"><Button type="button" variant="secondary" onPress={() => setEditing(null)}>Cancelar</Button><Button type="submit" variant="primary">Salvar alterações</Button></div></form></ActionDialog>}
