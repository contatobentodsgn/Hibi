...
export function RemindersScreen({ data, onEvent, onReminderStatusChange, onCreateReminder, onRenameReminder, onDeleteReminder, onEditReminderSchedule }: Props) {
  ...
  return <PixanoUiRoot className="reminders-screen">
    ...
    <div className="reminders-screen__layout">
      ...
      <section aria-labelledby="reminders-list-title"><div className="reminders-screen__heading"><h2 id="reminders-list-title">{filter === 'all' ? t('reminders.allTitle') : filter === 'important' ? t('reminders.important') : t('reminders.wellbeing')}</h2><p>{t('reminders.nextHint')}</p></div><Card className="reminders-screen__list">{visible.length ? <ul>{visible.map((reminder) => { const paused = reminder.status === 'paused'; return <li key={reminder.id}><span className="reminders-screen__icon" data-tone={reminder.category}><Bell size={16} /></span><div><strong>{reminder.title}</strong><span>{scheduleText(reminder, t)}</span></div><PixanoTag tone={paused ? 'neutral' : reminder.category === 'important' ? 'peach' : 'mint'}>{paused ? t('reminders.paused') : reminder.category === 'important' ? t('reminders.important') : t('reminders.wellbeing')}</PixanoTag><div className="reminders-screen__actions"><Button isIconOnly variant="ghost" aria-label={text('reminders.action.edit', { title: reminder.title })} onPress={() => beginEdit(reminder)}><CalendarClock size={16} /></Button><Button isIconOnly variant="ghost" aria-label={text(paused ? 'reminders.action.resume' : 'reminders.action.pause', { title: reminder.title })} onPress={() => { onReminderStatusChange(reminder.id, paused ? 'open' : 'paused'); onEvent(paused ? 'resume' : 'pause', reminder.title); }}>{paused ? <Play size={16} /> : <Pause size={16} />}</Button><Button isIconOnly variant="ghost" aria-label={text('reminders.action.delete', { title: reminder.title })} onPress={() => setDeleteId(reminder.id)}><Trash2 size={16} /></Button></div></li>; })}</ul> : <PixanoEmptyState icon={Bell} tone="mint" title={t('reminders.empty')} description={t('reminders.empty.description')} action={<Button variant="secondary" onPress={onCreateReminder}>{t('reminders.create')}</Button>} />}</Card></section>
