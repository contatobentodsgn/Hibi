...
function NoteDialog({ title, isOpen, onOpenChange, draft, setDraft, folders, onSubmit }: Readonly<{ title: string; isOpen: boolean; onOpenChange: (open: boolean) => void; draft: Draft; setDraft: (draft: Draft) => void; folders: readonly string[]; onSubmit: (event: FormEvent<HTMLFormElement>) => void }>) {
  const t = useT();
  const creating = title === 'Nova nota';
  return <ActionDialog trigger={<Button className="sr-only" aria-hidden="true">Abrir</Button>} isOpen={isOpen} onOpenChange={onOpenChange} title={title} description="O conteúdo permanece em texto simples e é salvo no workspace local."><form className="notes-screen__form" aria-label={creating ? 'Create note' : 'Edit note'} onSubmit={onSubmit}><label htmlFor={`${title}-title`}>{t('notes.titleField')}</label><input aria-label={t('notes.titleField')} id={`${title}-title`} autoFocus required value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /><label htmlFor={`${title}-content`}>{t('notes.contentField')}</label><textarea aria-label={t('notes.contentField')} id={`${title}-content`} rows={8} value={draft.content} onChange={(event) => setDraft({ ...draft, content: event.target.value })} /><label htmlFor={`${title}-folder`}>{t('notes.folderField')}</label><input aria-label={t('notes.folderField')} id={`${title}-folder`} list={`${title}-folders`} maxLength={FOLDER_NAME_MAX} value={draft.folder} onChange={(event) => setDraft({ ...draft, folder: event.target.value })} /><datalist id={`${title}-folders`}>{folders.map((folder) => <option key={folder} value={folder} />)}</datalist><div className="notes-screen__dialog-actions"><Button type="button" variant="secondary" onPress={() => onOpenChange(false)}>{t('notes.cancel')}</Button><Button type="submit" variant="primary" aria-label={creating ? t('notes.add') : t('notes.save')}>{creating ? t('notes.add') : t('notes.save')}</Button></div></form></ActionDialog>;
}
