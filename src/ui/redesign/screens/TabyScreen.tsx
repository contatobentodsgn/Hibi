  ...
  return <HibiUiRoot className="taby-screen">
    ...
    <div className="taby-screen__layout" data-history-open={historyOpen}>
      <aside id="taby-conversations" hidden={!historyOpen} className="taby-screen__history" role="region" aria-label="Conversas">
        ...
        <div className="taby-screen__capabilities"><strong>O Taby pode acessar</strong>{LOCAL_CAPABILITIES.filter((capability) => capability.status === 'available').map((capability) => <span key={capability.id}><Check size={12} aria-hidden="true" />{capability.label}</span>)}<small>External AI permanece indisponível no modo local.</small></div>
      </aside>

      <main className="taby-screen__conversation" aria-label="Conversa do Taby">
        ...
        <Card className="taby-screen__messages" aria-live="polite">
          <div className="taby-screen__assistant-intro"><span><Bot size={18} aria-hidden="true" /></span><div><strong>Taby</strong><p>Pronto para organizar o que importa agora.</p></div></div>
          {messages.length ? <ol>{messages.map((message, index) => <li key={`${message.role}-${message.at}-${index}`} data-role={message.role}><span>{message.role === 'user' ? 'Você' : 'Taby'}</span><p>{message.text}</p>{message.provenance && <small>{message.provenance}</small>}</li>)}</ol> : <HibiEmptyState icon={MessageCircle} tone="lavender" title="Comece por uma pergunta." description="O Taby consulta apenas seu espaço local e pede confirmação antes de qualquer alteração." />}
