...
const deadlineCopy = (
  ...
  t: (key: import("../../../i18n/dictionary").DictionaryKey) => string,
) => {
  if (!deadline) return t("tasks.deadline.none");
  const date = deadline.slice(0, 10).split("-").reverse().join("/");
  if (state === "overdue")
    return t("tasks.deadline.overdue").replace("{date}", date);
  if (state === "today")
    return t("tasks.deadline.today").replace("{time}", deadline.slice(11, 16));
...
const folderLabel = (
  folder: string | null,
  t: (key: import("../../../i18n/dictionary").DictionaryKey) => string,
) => (folder === null ? t("tasks.allFolders") : folder || t("tasks.noFolder"));
...
export function TasksScreen({
  ...
  const t = useT();
  const copy = (
    key: import("../../../i18n/dictionary").DictionaryKey,
  ...
  const activeFolder =
    folder !== null && folders.some((entry) => entry.name === folder)
      ? folder
      : null;
  const visibleFolders = folders.filter(
    (entry) => entry.tasks > 0 || entry.name === activeFolder,
  );
  ...
  const visibleTasks = useMemo(
    () =>
      [...data.tasks]
        ...
        .filter(
          (task) =>
            (scope === "all" || isOpen(task) || task.id === selectedId) &&
            (activeFolder === null || folderOf(task) === activeFolder),
        )
        .sort((left, right) =>
          sortByDeadline
            ? (left.deadline ?? "9999").localeCompare(
                right.deadline ?? "9999",
              ) || left.title.localeCompare(right.title)
            : 0,
        ),
    [activeFolder, data.tasks, scope, sortByDeadline],
  );
  ...
  const chooseFolder = (next: string | null) => {
    ...
    onEvent(
      "filter",
      `Tarefas · Pasta · ${next === null ? "todas" : next || "sem pasta"}`,
    );
  };
  ...
  return (
    <HibiUiRoot className="tasks-screen">
      <SectionHeader
        ...
        actions={
          <ActionDialog
            ...
            title={t("tasks.nextStep")}
            description={copy("tasks.createdIn", {
              folder: folderLabel(activeFolder, t).toLowerCase(),
            })}
          >
      ...
      <div className="tasks-screen__layout">
        <aside
          className="tasks-screen__sidebar"
          aria-label={t("tasks.filters")}
        >
          <div className="tasks-screen__filter-group">
            <span className="tasks-screen__filter-heading">
              <ListFilter size={14} aria-hidden="true" />
              {t("tasks.view")}
            </span>
            <button
              type="button"
              data-active={scope === "open"}
              onClick={() => {
                setScope("open");
                onEvent("filter", "Tarefas · Abertas");
              }}
            >
              <Circle size={15} aria-hidden="true" />
              {t("tasks.open")} <span>{openTaskCount}</span>
            </button>
            <button
              type="button"
              ...
              onClick={() => {
                setScope("all");
                onEvent("filter", "Tarefas · Todas");
              }}
            >
              <CheckCheck size={15} aria-hidden="true" />
              {t("tasks.all")} <span>{data.tasks.length}</span>
            </button>
          </div>
          <div className="tasks-screen__filter-group">
            <span className="tasks-screen__filter-heading">
            ...
            <button
              type="button"
              aria-label={`${t("tasks.folders")} · ${t("tasks.all")}`}
            ...
            {visibleFolders.map((entry) => (
              <button
                key={entry.name || "none"}
                type="button"
                aria-label={`${t("tasks.folders")} · ${entry.name || t("tasks.noFolder")} ${entry.tasks}`}
                aria-pressed={activeFolder === entry.name}
                data-active={activeFolder === entry.name}
                onClick={() => chooseFolder(entry.name)}
              >
                {entry.name || t("tasks.noFolder")} <span>{entry.tasks}</span>
              </button>
            ))}
          </div>
        </aside>
        ...
        <section
          className="tasks-screen__main"
          aria-labelledby="tasks-list-title"
        >
          <div className="tasks-screen__list-header">
            <div>
              <h2 id="tasks-list-title">{folderLabel(activeFolder, t)}</h2>
              <p>
                {copy("tasks.count", {
                  count: visibleTasks.length,
                  suffix: visibleTasks.length === 1 ? "" : "s",
                })}
              </p>
            </div>
            <Button
              ...
              onPress={() => {
                ...
                onEvent("sort", t("tasks.byDeadline"));
              }}
            >
            ...
            {visibleTasks.length ? (
              <ul className="tasks-screen__list">
                {visibleTasks.map((task) => {
                  ...
                  const deadlineState =
                    rhythm.deadlineStateById[task.id] ?? "none";
                  return (
                    <li
                      className="task-row"
                      key={task.id}
                      data-deadline={deadlineState}
                    >
                      <button
                        type="button"
                        className="tasks-screen__check"
                        aria-label={`${t(completed ? "tasks.reopen" : "tasks.complete")} ${task.title}`}
                      ...
                      <TaskDetailsPanel
                        ...
                        trigger={
                          <Button
                            className="tasks-screen__task-trigger"
                            variant="ghost"
                            onPress={() => setSelectedId(task.id)}
                          >
                            <span className="tasks-screen__task-copy">
                        ...
                        onOpenChange={(open) =>
                          setSelectedId(open ? task.id : null)
                        }
                      ...
                      >
                        <TaskPanelActions
                          ...
                          onEditDeadline={() => {
                            setSelectedId(null);
                            onEditTaskDeadline?.(task.id);
                          }}
                          onDeleteOpenChange={setDeleteOpen}
                          onDelete={() => {
                            onDeleteTask?.(task.id);
                            onEvent("delete", task.title);
                            setDeleteOpen(false);
                            setSelectedId(null);
                          }}
                        />
              ...
              <div className="tasks-screen__empty">
                <HibiEmptyState
                  ...
                  action={
                    ...
                    >
                      {activeFolder !== null
                        ? t("tasks.createInFolder").replace(
                            "{folder}",
                            folderLabel(activeFolder, t),
                          )
                        : t("tasks.create")}
                    </Button>
                  }
        ...
        <Card
          className="tasks-screen__summary"
          aria-label={t("tasks.summary.aria")}
        >
          <h2>{t("tasks.summary.title")}</h2>
          <p>{t("tasks.summary.detail")}</p>
          <div>
            <span>
              <strong>{rhythm.overdue}</strong>
              {t("tasks.overdue")}
            </span>
            <span>
              <strong>{rhythm.dueToday}</strong>
              {t("tasks.today")}
            </span>
            <span>
              <strong>{rhythm.withoutDeadline}</strong>
              {t("tasks.noDeadline")}
            </span>
          </div>
          {rhythm.next ? (
            <p className="tasks-screen__next">
              <Clock3 size={15} aria-hidden="true" />
              <span>
                {t("tasks.next")} <strong>{rhythm.next.title}</strong>
              </span>
            </p>
          ) : (
            <p className="tasks-screen__next">
...
function TaskPanelActions({
  ...
  return (
    <>
      {editingTitle ? (
        <form className="tasks-screen__edit-form" onSubmit={onRename}>
          <label htmlFor={`task-title-${task.id}`}>
            {t("tasks.titleField.short")}
          </label>
          <input
          ...
          />
          <div>
            <Button type="button" variant="secondary" onPress={onCancelEdit}>
              {t("tasks.cancel")}
            </Button>
            <Button type="submit" variant="primary">
              {t("habits.save")}
        ...
        <Button variant="secondary" onPress={onStartEdit}>
          <Pencil size={15} aria-hidden="true" />
          {t("tasks.editTitle")}
        </Button>
      )}
      <Button variant="secondary" onPress={onEditDeadline}>
        <CalendarClock size={15} aria-hidden="true" />
        {t("tasks.editDeadline")}
      </Button>
      <Button
      ...
      <ActionDialog
        ...
        description={t("tasks.deleteDescription").replace(
          "{title}",
          task.title,
        )}
      >
        <div className="tasks-screen__confirm-actions">
