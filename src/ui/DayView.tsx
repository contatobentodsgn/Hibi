...
export function DayView({ data, onEvent, onCreateBlock, onDeleteBlock }: Props) {
  ...
  const [dayDate, setDayDate] = useState(() => referenceDate(data));
  const dayLabel = new Date(`${dayDate}T12:00:00${OFFSET}`).toLocaleDateString('en-GB', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }).toUpperCase().replace(',', ' ·');
  ...
  const moveDay = (amount: number) => { const date = new Date(`${dayDate}T12:00:00${OFFSET}`); date.setDate(date.getDate() + amount); const next = date.toISOString().slice(0, 10); setDayDate(next); onEvent('navigation', amount < 0 ? 'Previous day' : 'Next day'); };
