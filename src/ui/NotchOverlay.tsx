...
type OverlayPresentation = { requestId: string; kind: string; text: string | null; actions: readonly { id: string; label: string }[]; interaction: 'passthrough' | 'capture' };
export const notchMediaFor = (kind: string) => ({ listening: companionAssets.animations.notch.listeningLoop, thinking: companionAssets.animations.notch.searchingLoop, acting: companionAssets.animations.notch.creatingTaskLoop, result: companionAssets.animations.notch.tabyResponseReadyLoop, error: companionAssets.animations.notch.disappointed, reminder: companionAssets.animations.notch.waiting01 } as const)[kind as 'listening'] ?? companionAssets.animations.notch.idle01Loop;

export function NotchOverlay() {
  const [presentation, setPresentation] = useState<OverlayPresentation | null>(null);
  ...
  return <main className="notch-overlay" role={presentation.actions.length ? 'dialog' : 'status'} aria-label={presentation.kind} data-interaction={presentation.interaction}>
    ...
    {presentation.actions.map((action) => <button key={action.id} onClick={() => void window.hibiDesktop?.hideNotch?.(presentation.requestId)}>{action.label}</button>)}
  </main>;
}
