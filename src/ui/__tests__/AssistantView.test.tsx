...
describe('AssistantView capability boundaries', () => {
  ...
  it('maps assistant stages, confirmations, results, and failures to companion events', () => {
    ...
    expect(companionEventFor('result', 'r-1', 'Done', 10)).toMatchObject({ type: 'ai.result', requestId: 'r-1', expiresInMs: 8_000 });
...
describe('AssistantView conversations', () => {
  ...
  it('renders the conversations it is given', () => {
    ...
    expect(markup).toContain('aria-label="Status da conversa do assistente"');
    expect(markup).toContain('Mensagens nesta conversa');
  });
