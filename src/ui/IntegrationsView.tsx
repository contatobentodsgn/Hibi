...
type Props = Readonly<{ onEvent: (action: string, detail: string, result?: string) => void; localRecords?: readonly LocalImportRecord[]; onApplyImport?: (candidate: ImportCandidate, decision: ImportDecision, localId?: string, connectorId?: string) => void }>
...
export function IntegrationsView({ onEvent, localRecords = [], onApplyImport }: Props) {
  ...
  const saveSettings = async (connectorId: string, patch: Partial<ConnectorSettings>) => {
    try {
    ...
    } catch (error) { setNotice(error instanceof Error ? error.message : 'Could not save this connector configuration.'); onEvent('connector-settings', connectorId, 'fail') }
  }
