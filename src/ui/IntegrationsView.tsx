...
export function IntegrationsView({ onEvent, localRecords = [], localTasks = [], localBlocks = [], onApplyImport, onApplyNotion }: Props) {
  ...
  const openSettings = async (connectorId: string) => {
    ...
    try {
      const value = await window.hibiDesktop?.getConnectorSettings?.(connectorId)
      if (value) setSettings((current) => ({ ...current, [connectorId]: value }))
      if (oauthConnectors.includes(connectorId)) {
        const hasSecret = await window.hibiDesktop?.hasOauthClientSecret?.(connectorId)
  ...
  const saveClientSecret = async (connectorId: string) => {
    ...
    try {
      const result = await window.hibiDesktop?.saveOauthClientSecret?.(connectorId, clientSecretDraft)
      setClientSecretDraft('')
  ...
  const clearClientSecret = async (connectorId: string) => {
    try {
      const result = await window.hibiDesktop?.clearOauthClientSecret?.(connectorId)
      setClientSecretDraft('')
      if (result) setClientSecretState((current) => ({ ...current, [connectorId]: result.hasClientSecret }))
