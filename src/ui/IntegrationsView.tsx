...
type Props = Readonly<{
  ...
  localRecords?: readonly LocalImportRecord[];
  localTasks?: readonly Task[];
  localBlocks?: readonly ScheduleBlock[];
  onApplyImport?: (
...
export function IntegrationsView({
  ...
  const refresh = async () => {
    try {
      const [nextStatus, nextAudit] = await Promise.all([
        window.hibiDesktop?.listIntegrationStatus?.(),
        window.hibiDesktop?.listIntegrationAudit?.(),
      ]);
      ...
      setNotice("Could not refresh integration status.");
      onEvent("integrations-refresh", "Integration status", "fail");
    }
  };
  useEffect(() => {
    void refresh();
  ...
  useEffect(() => {
    void window.hibiDesktop
      ?.getConnectorSettings?.("notion")
      .then((value) => {
        if (value) setSettings((current) => ({ ...current, notion: value }));
      })
      .catch(() => undefined);
  }, []);
  useEffect(() => {
    void window.hibiDesktop
      ?.getWebhookStatus?.()
      .then((value) => {
        if (value) setWebhook(value);
      })
      .catch(() => undefined);
  }, []);
  const revoke = async (connectorId: string) => {
    try {
      const next = await window.hibiDesktop?.revokeIntegration?.(connectorId);
      if (next)
        setStatus((current) =>
          current.map((entry) => (entry.id === next.id ? next : entry)),
        );
      setNotice("Integration credential revoked from Keychain.");
      onEvent("integration-revoke", connectorId, "pass");
    } catch {
      setNotice("Could not revoke this integration.");
      onEvent("integration-revoke", connectorId, "fail");
    }
  };
  const connect = async (connectorId: string) => {
    if (!credential.trim()) {
      setNotice("Paste an access token supplied by the service first.");
      return;
    }
    try {
      const next = await window.hibiDesktop?.connectIntegration?.(
        connectorId,
        credential,
      );
      setCredential("");
      ...
      setNotice("Could not save this access token.");
      onEvent("integration-connect", connectorId, "fail");
    }
  };
  const importFile = async (file: File) => {
    try {
      const candidates = parseImportCandidates(file.name, await file.text());
      setPreviewSource("file-import");
      setPreview(buildImportPreview(localRecords, candidates, "file-import"));
      setNotice(
        `${candidates.length} items are ready for review; nothing has been imported yet.`,
      );
      onEvent("integration-import-preview", file.name, "pass");
    } catch (error) {
      setPreview([]);
      setNotice(
      ...
      onEvent("integration-import-preview", file.name, "fail");
    }
  };
  const applyImport = (item: ImportPreviewItem, decision: ImportDecision) => {
    onApplyImport?.(item, decision, item.localId, previewSource);
    setPreview((current) =>
      current.filter((entry) => entry.remoteId !== item.remoteId),
    );
    setNotice(
      decision === "skip" || decision === "keep-local"
        ? "Import decision recorded without changing the local task."
        : "Import decision applied to the local workspace.",
    );
  ...
  const toggleApi = async () => {
    try {
      if (api) {
        await window.hibiDesktop?.stopLocalApi?.();
        setApi(null);
        setNotice("Local API stopped.");
        return;
      }
      const started = await window.hibiDesktop?.startLocalApi?.();
      if (!started) {
        setNotice("Local API is available in the desktop app.");
        return;
      }
      setApi({ origin: started.origin });
      setNotice(
        "Local API started. Its bearer token remains in Keychain and is never shown here.",
      );
      onEvent("local-api-start", started.origin, "pass");
    } catch {
      setNotice("Could not change the Local API state.");
      onEvent("local-api", "toggle", "fail");
    }
  };
  useEffect(() => {
    void Promise.all(
      initial.map(
        async (connector) =>
          [
            connector.id,
            (await window.hibiDesktop?.isOauthSupported?.(connector.id)) ===
              true,
          ] as const,
      ),
    )
      .then((entries) =>
        setOauthConnectors(
          entries.filter(([, supported]) => supported).map(([id]) => id),
        ),
      )
      .catch(() => undefined);
  }, []);
  const settingsFor = (connectorId: string) =>
    settings[connectorId] ?? EMPTY_SETTINGS;
  ...
  const refreshOauthSupport = async (connectorId: string) => {
    const supported =
      (await window.hibiDesktop?.isOauthSupported?.(connectorId)) === true;
    setOauthConnectors((current) =>
      supported
        ? current.includes(connectorId)
          ? current
          : [...current, connectorId]
        : current.filter((entry) => entry !== connectorId),
    );
  };
  const saveConnection = (
    ...
    void saveSettings(connectorId, patch)
      .then(() => refreshOauthSupport(connectorId))
      .catch(() => undefined);
  };
  const openSettings = async (connectorId: string) => {
    ...
    try {
      const value =
        await window.hibiDesktop?.getConnectorSettings?.(connectorId);
      if (value)
        setSettings((current) => ({ ...current, [connectorId]: value }));
    } catch {
      setNotice("Could not read this connector configuration.");
    }
  };
  const saveSettings = async (
    ...
    try {
      const value = await window.hibiDesktop?.saveConnectorSettings?.(
        connectorId,
        patch,
      );
      if (value) {
        setSettings((current) => ({ ...current, [connectorId]: value }));
        setNotice("Connector configuration saved on this Mac.");
        onEvent("connector-settings", connectorId, "pass");
        return value;
      }
      setNotice("Connector configuration saved on this Mac.");
      onEvent("connector-settings", connectorId, "pass");
      ...
      setNotice(
        error instanceof Error
          ? error.message
          : "Could not save this connector configuration.",
      );
      onEvent("connector-settings", connectorId, "fail");
      throw error;
    }
  };
  const authorize = async (connectorId: string) => {
    if (!settingsFor(connectorId).clientId) {
      setNotice(
        "Add the client identifier supplied by the service before authorizing.",
      );
      return;
    }
    setBusy(connectorId);
    try {
      const result =
        await window.hibiDesktop?.authorizeIntegration?.(connectorId);
      // A atualização de status vem antes: refresh() também escreve no aviso.
      if (result) {
        await refresh();
        setNotice(
          `Authorized. Tokens stay in Keychain${result.hasRefreshToken ? " and can be refreshed" : ""}.`,
        );
      }
      ...
      onEvent("integration-authorize", connectorId, "fail");
    } finally {
      setBusy(null);
    }
  };
  const testConnection = async (connectorId: string) => {
    ...
    try {
      const result =
        await window.hibiDesktop?.testIntegrationConnection?.(connectorId);
      setNotice(
        result
          ? `${result.ok ? "Connection test passed" : "Connection test failed"}: ${result.detail}`
          : "Connection tests are available in the desktop app.",
      );
      onEvent("integration-test", connectorId, result?.ok ? "pass" : "fail");
    } catch (error) {
      setNotice(
        error instanceof Error
          ? error.message
          : "Could not test this connection.",
      );
      onEvent("integration-test", connectorId, "fail");
    } finally {
      setBusy(null);
    }
  };
  const loadTargets = async (connectorId: string) => {
    ...
    try {
      const value =
        await window.hibiDesktop?.listIntegrationImportTargets?.(connectorId);
      setTargets((current) => ({ ...current, [connectorId]: value ?? [] }));
      setNotice(
        value?.length
          ? `${value.length} import targets available.`
          : "No import target was returned.",
      );
    } catch (error) {
      setNotice(
        error instanceof Error
          ? error.message
          : "Could not list import targets.",
      );
    } finally {
      setBusy(null);
    }
  ...
  const importFromConnector = async (connectorId: string) => {
    ...
    try {
      const candidates =
        await window.hibiDesktop?.listIntegrationImportCandidates?.(
          connectorId,
        );
      if (!candidates) {
        setNotice("Connector imports are available in the desktop app.");
        return;
      }
      // Leitura apenas: a prévia é a mesma da importação por arquivo e nada entra
      // no workspace antes de uma decisão por item.
      setPreviewSource(connectorId);
      setPreview(buildImportPreview(localRecords, candidates, connectorId));
      setNotice(
        candidates.length
          ? `${candidates.length} items read from ${connectorId}; nothing has been imported yet.`
          : `No item was returned by ${connectorId}.`,
      );
      ...
      setNotice(
        error instanceof Error
          ? error.message
          : "Could not read from this connector.",
      );
      onEvent("integration-import-read", connectorId, "fail");
    } finally {
      setBusy(null);
    }
  };
  const toggleTarget = (
    ...
    const next = current.some((entry) => entry.id === target.id)
  ...
  const configureWebhook = async () => {
    if (!webhookSecret.trim()) {
      setNotice("Paste a webhook signing secret first.");
      return;
    }
    try {
      const value = await window.hibiDesktop?.configureWebhook?.(webhookSecret);
      setWebhookSecret("");
      if (value) setWebhook(value);
      setNotice("Webhook secret saved in Keychain.");
      onEvent("webhook-configure", "Local webhook", "pass");
    } catch {
      setNotice("Could not configure the local webhook.");
    }
  };
