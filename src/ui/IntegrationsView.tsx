...
export function IntegrationsView({ onEvent, localRecords = [], localTasks = [], onApplyImport, onApplyNotion }: Props) {
  ...
  const refreshOauthSupport = async (connectorId: string) => {
    const supported = await window.hibiDesktop?.isOauthSupported?.(connectorId) === true
