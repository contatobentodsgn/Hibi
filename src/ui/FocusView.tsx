...
export function FocusView({ autoStart, onAutoStarted, onEvent, onFocusStarted, onFocusCompleted, onFocusLifecycle, onFocusWindowChange, mode = 'focus', onModeChange, sessionMinutes, awayBehavior, idleMinutes, focusLoopAnimation, activity, presence, onCompanionEvent, subscribeCompanionActions }: Props) {
  ...
  return <HibiUiRoot className={`focus-view focus-view--redesign ${onBreak ? 'focus-view--break' : ''}`}><section className="focus-view__session" aria-label={onBreak ? 'Sessão de pausa' : 'Sessão de foco'}><div className="focus-view__intro"><div><p className="eyebrow">{onBreak ? t('focus.breakEyebrow') : 'FOCO · SESSÃO LOCAL'}</p><h1>{heading}</h1><p className="subhead">{subhead}</p></div><CompanionAnimation state={running && !onBreak ? resolveFocusLoopAnimationId(focusLoopAnimation, focusMood) : 'idle'} label={onBreak ? t('focus.breakCompanion') : 'Focus companion'} /></div><div className={`focus-ring ${running ? 'is-running' : ''}`}><span>{time}</span><small>{t('focus.minutes')}</small></div><div className="focus-view__actions"><button className="primary focus-button" onClick={toggle}>{action}</button><button className="outline" disabled={running} onClick={() => onModeChange?.(onBreak ? 'focus' : 'break')}>{onBreak ? t('focus.backToFocus') : t('focus.takeBreak')}</button></div>{promptCard}{pausedAway && !running && <p className="muted" role="status">{t('focus.presence.pausedAway')}</p>}{notCompleted && !running && !onBreak && <p className="muted focus-not-completed" role="status">{notCompletedText(language, notCompleted.presentMinutes, notCompleted.plannedMinutes)}</p>}<div className="focus-options" aria-label={onBreak ? 'Duração da pausa' : 'Duração do foco'}>{DURATIONS.map((minutes) => <button key={minutes} className={`filter ${duration === minutes ? 'active' : ''}`} aria-pressed={DURATIONS.length > 1 ? duration === minutes : undefined} disabled={running} onClick={() => chooseDuration(minutes)}>{onBreak ? `${minutes}m` : `${minutes}m focus`}</button>)}</div><FocusAtelierSummary onBreak={onBreak} running={running} completedToday={completedToday} awayBehavior={behavior} />{!onBreak && <p className="muted focus-view__quiet">{t('focus.quiet')}</p>}</section></HibiUiRoot>;
}
