...
import { FocusAtelierSummary } from './FocusAtelierSummary';
import { PixanoUiRoot } from './redesign/components/PixanoUiRoot';
import './redesign/screens/focus-screen.css';
...
/** O aviso da sessão que chegou ao fim sem presença bastante para contar como concluída. */
export const notCompletedText = (locale: Locale, presentMinutes: number, plannedMinutes: number): string => {
  const minutes = (count: number) => pluralize(locale, count, 'focus.count.minute.one', 'focus.count.minute.other');
...
export function FocusView({ autoStart, onAutoStarted, onEvent, onFocusStarted, onFocusCompleted, onFocusLifecycle, onFocusWindowChange, mode = 'focus', onModeChange, sessionMinutes, awayBehavior, idleMinutes, focusLoopAnimation, activity, presence, onCompanionEvent, subscribeCompanionActions }: Props) {
  ...
  // A última sessão chegou ao zero sem presença confirmada em metade da duração: a tela diz por que ela não
  // contou, em vez de a sessão sumir calada. Some quando outra sessão começa.
  const [notCompleted, setNotCompleted] = useState<Readonly<{ presentMinutes: number; plannedMinutes: number }> | null>(null);
  ...
  const emitLifecycle = (action: FocusLifecycleAction, pending: FocusAbsence | undefined = pendingAbsence()): FocusLifecycleEvent | undefined => {
  ...
  useEffect(() => {
    ...
    replacePrompt(null);
    if (onBreak) { onEvent('break-complete', 'Completed break', 'pass'); return; }
    const ended = emitLifecycle('complete', pending);
    // Sem presença confirmada em metade da duração a sessão foi rebaixada a cancelada: nada de comemoração
    // no companion nem de focus-complete, e a tela avisa quanto da sessão teve alguém na frente do Mac.
    if (ended?.type === 'cancelled') {
      setNotCompleted({ presentMinutes: ended.focusedMinutes ?? 0, plannedMinutes: duration });
      onEvent('focus-cancel', 'Focus session did not count as completed: presence confirmed for less than half of it', 'cancelled');
      return;
    }
    onFocusCompleted?.();
    onEvent('focus-complete', 'Completed focus session', 'pass');
  ...
  const toggle = () => {
    ...
    if (starting) { setNotCompleted(null); onFocusStarted?.(); }
  ...
  // "Iniciar foco" pelo Assistant (ou pela voz) só abria esta tela e anunciava no notch uma sessão que não
  ...
  const heading = onBreak ? (running ? t('focus.breakTitle') : t('focus.breakReady')) : (running ? t('focus.runningTitle') : t('focus.ready'));
  const subhead = onBreak ? (running ? t('focus.breakRunning') : `${duration} ${t('focus.breakOnClock')}`) : (running ? t('focus.runningDetail') : t('focus.readyDetail').replace('{duration}', () => String(duration)));
  const action = onBreak ? (running ? t('focus.stopBreak') : t('focus.startBreak')) : (running ? t('focus.pause') : t('focus.start'));
  ...
  return <PixanoUiRoot className={`focus-view focus-view--redesign ${onBreak ? 'focus-view--break' : ''}`}><section className="focus-view__session" aria-label={onBreak ? t('focus.breakTitle') : t('nav.focus')}><div className="focus-view__intro"><div><p className="eyebrow">{onBreak ? t('focus.breakEyebrow') : t('focus.eyebrow')}</p><h1>{heading}</h1><p className="subhead">{subhead}</p></div><CompanionAnimation state={running && !onBreak ? resolveFocusLoopAnimationId(focusLoopAnimation, focusMood) : 'idle'} label={onBreak ? t('focus.breakCompanion') : t('focus.companion')} /></div><div className={`focus-ring ${running ? 'is-running' : ''}`}><span>{time}</span><small>{t('focus.minutes')}</small></div><div className="focus-view__actions"><button className="primary focus-button" onClick={toggle}>{action}</button><button className="outline" disabled={running} onClick={() => onModeChange?.(onBreak ? 'focus' : 'break')}>{onBreak ? t('focus.backToFocus') : t('focus.takeBreak')}</button></div>{promptCard}{pausedAway && !running && <p className="muted" role="status">{t('focus.presence.pausedAway')}</p>}{notCompleted && !running && !onBreak && <p className="muted focus-not-completed" role="status">{notCompletedText(language, notCompleted.presentMinutes, notCompleted.plannedMinutes)}</p>}<div className="focus-options" aria-label={onBreak ? t('focus.breakTitle') : t('nav.focus')}>{DURATIONS.map((minutes) => <button key={minutes} className={`filter ${duration === minutes ? 'active' : ''}`} aria-pressed={DURATIONS.length > 1 ? duration === minutes : undefined} disabled={running} onClick={() => chooseDuration(minutes)}>{onBreak ? `${minutes}m` : t('focus.duration').replace('{duration}', () => String(minutes))}</button>)}</div><FocusAtelierSummary onBreak={onBreak} running={running} completedToday={completedToday} awayBehavior={behavior} />{!onBreak && <p className="muted focus-view__quiet">{t('focus.quiet')}</p>}</section></PixanoUiRoot>;
}
