...
export function TabyView({ turn, conversations }: Props) {
  ...
  return <div className="view">
    ...
    <div className="quick-input"><input disabled={isRunning} value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') send(); }} placeholder="Pergunte ou peça uma ação" aria-label="Pergunte ou peça uma ação" /><button className="primary" disabled={isRunning} onClick={send}>Send</button>{isRunning && <button className="outline" onClick={turn.stop}>Parar</button>}</div>
  </div>;
}
