...
export function TabyView({ data, onEvent }: { data: StudyData; onEvent: (action: string, detail: string, result?: string) => void }) {
  ...
  const ask = () => {
    ...
    let answer = `Tenho ${data.tasks.length} tarefas, ${data.reminders.length} lembretes e ${data.blocks.length} blocos agendados.`;
    ...
    setMessages((current) => [...current, { role: 'user', text: question }, { role: 'assistant', text: answer }]); setInput(''); onEvent('assistant-query', question, 'local');
  };
