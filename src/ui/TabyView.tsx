...
type Props = { data: StudyData; runtime: AiTurnRuntime; onEvent: (action: string, detail: string, result?: string) => void; onCompanionError?: (text: string) => void };
...
export function TabyView({ data, runtime, onEvent, onCompanionError }: Props) {
  ...
  const resolvePending = async (actionId: 'confirm' | 'cancel') => {
    ...
    } catch (error) { const text = error instanceof Error ? error.message : 'A ação não pôde ser executada.'; setMessages((current) => [...current, { role: 'assistant', text }]); onEvent('assistant-action', text, 'blocked'); onCompanionError?.(text); }
  };
  ...
  const ask = async () => {
    ...
    } catch (error) { const text = error instanceof Error ? error.message : 'Não foi possível concluir essa solicitação.'; setMessages((current) => [...current, { role: 'assistant', text }]); onEvent('assistant-query', text, 'failed'); onCompanionError?.(text); }
  };
