...
import { NotesAtelierSummary } from './NotesAtelierSummary';
...
// `editor` só existe enquanto uma nota está sendo editada — a criação usa o formulário inline
// sempre visível (ver NoteForm com titleId=NEW_NOTE_TITLE_ID), nunca este diálogo.
type Editor = { id: string; title: string; content: string; folder: string };
// Único id do campo Title do formulário de criação — usado tanto no titleId do NoteForm quanto no
// clique de "+ New note", para nunca divergirem.
...
// `editor` aqui aceita tanto os valores iniciais da criação (sem id) quanto uma nota em edição.
function NoteForm({ editor, folders, onCancel, onSubmit, titleId }: { editor: Omit<Editor, 'id'> & { id?: string }; folders: readonly string[]; onCancel?: () => void; onSubmit: (title: string, content: string, folder: string) => void; titleId: string }) {
...
export function NotesView({ data, onCreate, onUpdate, onDelete, initialFolder = null }: Props) {
  ...
  const visibleFolders = allFolders.filter((entry) => entry.notes > 0 || entry.name === activeFolder);
