...
// `now` existe para o teste: a tela lê o relógio real, e sem injetar o instante não dá para afirmar
// o que "agora" e "a seguir" mostram. É o mesmo recurso que a tela de Hábitos já usa.
type Props = { data: StudyData; onEvent: (action: string, detail: string, result?: string) => void; onNavigate: (route: any) => void; onOpenCommands?: () => void; now?: Date };

export function HomeView({ data, onEvent, onNavigate, onOpenCommands, now = new Date() }: Props) {
  ...
  const next = rhythm.later.slice(0, 3);
