...
function createCalendarSyncService({ eventKit, integrations, settings, calendarSettings = null, now = () => new Date().toISOString(), randomId = () => require('node:crypto').randomUUID() } = {}) {
  ...
  const persisted = () => calendarSettings?.get?.() ?? { calendars: [], sources: [] };
  const persistedCalendars = () => Array.isArray(persisted().calendars)
    ? persisted().calendars.filter((calendar) => boundedText(calendar?.id) && isMode(calendar?.mode))
    : [];
  const modeFor = (id) => persistedCalendars().find((calendar) => calendar.id === id)?.mode ?? 'read-only';
  const sourceLastSyncedAt = (id) => Array.isArray(persisted().sources) ? persisted().sources.find((source) => source?.id === id)?.lastSyncedAt : undefined;
  const withSyncMetadata = (source) => ({ ...source, ...(boundedText(sourceLastSyncedAt(source.id)) ? { lastSyncedAt: sourceLastSyncedAt(source.id) } : {}) });
  const markSourcesSynced = (ids) => {
    if (!calendarSettings || typeof calendarSettings.save !== 'function') return;
    const state = persisted();
    const nowValue = now();
    const latest = new Map((Array.isArray(state.sources) ? state.sources : []).map((source) => [source.id, source]));
    for (const id of ids) latest.set(id, { id, lastSyncedAt: nowValue });
    calendarSettings.save({ calendars: persistedCalendars(), sources: [...latest.values()] });
  };
  const prepared = new Map();
  const validBlock = (block) => block && typeof block === 'object'
    && boundedText(block.id, 240) && boundedText(block.title, 240)
    && parseInstant(block.startsAt) && parseInstant(block.endsAt)
  ...
  const appleCalendars = () => {
    ...
    return (eventKit.listCalendars() ?? []).flatMap((calendar) => {
      ...
      return [{ id, sourceId: 'apple', label: `${calendar.label}${suffix}`.slice(0, 240), mode: modeFor(id) }];
    });
  };
  ...
  return {
    async getState() {
      ...
      return {
        ...
        calendars: [...appleCalendars(), ...selectedGoogle().map((calendar) => {
          const id = `google:${calendar.id}`;
          return { id, sourceId: 'google', label: boundedText(calendar.label) ? calendar.label : calendar.id, mode: modeFor(id) };
        })],
        conflicts: [],
      };
    },
    ...
    async saveCalendarMode(input = {}) {
      if (!calendarSettings || typeof calendarSettings.save !== 'function') throw new Error('Calendar sync settings are unavailable.');
      if (!boundedText(input.id) || !isMode(input.mode)) throw new Error('Calendar sync mode is invalid.');
      const current = persistedCalendars();
      const next = [...current.filter((calendar) => calendar.id !== input.id), { id: input.id, mode: input.mode }];
      calendarSettings.save({ calendars: next, sources: Array.isArray(persisted().sources) ? persisted().sources : [] });
      return this.getState();
    },
    async preparePublish(input = {}) {
      if (!boundedText(input.calendarId) || !validBlock(input.block)) throw new Error('Calendar block is invalid.');
      if (modeFor(input.calendarId) !== 'bidirectional') throw new Error('Choose bidirectional mode before publishing a Hibi block.');
      const block = { id: input.block.id, title: input.block.title, startsAt: input.block.startsAt, endsAt: input.block.endsAt, allDay: input.block.allDay === true };
      if (input.calendarId.startsWith('google:')) {
        if (typeof integrations.prepareAction !== 'function') throw new Error('Google Calendar publishing is unavailable.');
        const calendarId = input.calendarId.slice('google:'.length);
        const action = await integrations.prepareAction({ connectorId: 'google-calendar', kind: 'calendar.create', payload: { calendarId, title: block.title, startsAt: block.startsAt, endsAt: block.endsAt, allDay: block.allDay } });
        if (!boundedText(action?.id, 240) || !boundedText(action?.confirmationId, 240)) throw new Error('Google Calendar action preparation is invalid.');
        prepared.set(action.id, { external: true, confirmationId: action.confirmationId });
        return { id: action.id, confirmationId: action.confirmationId, requiresConfirmation: true, calendarId: input.calendarId, summary: block.title };
      }
      if (!input.calendarId.startsWith('apple:')) throw new Error('Publishing to this calendar is unavailable.');
      const calendarId = input.calendarId.slice('apple:'.length);
      const calendar = appleCalendars().find((entry) => entry.id === input.calendarId);
      if (!calendar) throw new Error('The selected calendar is unavailable.');
      const nativeCalendar = (eventKit.listCalendars() ?? []).find((entry) => entry?.id === calendarId);
      if (nativeCalendar?.writable !== true || typeof eventKit.saveEvent !== 'function') throw new Error('The selected calendar cannot be changed.');
      const id = `calendar-${randomId()}`;
      const confirmationId = `calendar-confirm-${randomId()}`;
      prepared.set(id, { confirmationId, calendarId, block });
      return { id, confirmationId, requiresConfirmation: true, calendarId: input.calendarId, summary: input.block.title };
    },
    async executeApproved(input = {}) {
      const action = prepared.get(input.actionId);
      if (!action || input.confirmationId !== action.confirmationId) throw new Error('A matching confirmation is required before publishing this calendar block.');
      prepared.delete(input.actionId);
      if (action.external) {
        if (typeof integrations.executeApproved !== 'function') throw new Error('Google Calendar publishing is unavailable.');
        const result = await integrations.executeApproved({ actionId: input.actionId, confirmationId: input.confirmationId });
        if (result?.ok !== true || !boundedText(result?.remoteId, 240)) throw new Error('Calendar event could not be created.');
        return { remoteId: result.remoteId };
      }
      const remote = eventKit.saveEvent({ calendarId: action.calendarId, title: action.block.title, start: action.block.startsAt, end: action.block.endsAt, allDay: action.block.allDay });
      const result = await Promise.resolve(remote);
      if (!boundedText(result?.id, 240)) throw new Error('Calendar event could not be created.');
    ...
    async readEvents(input = {}) {
      ...
      markSourcesSynced([...new Set(calendars.filter((calendar) => calendar?.sourceId === 'apple' || calendar?.sourceId === 'google').map((calendar) => calendar.sourceId))]);
      return events;
    },
  };
}
