...
function createMacVoiceAdapter({ spawnProcess = spawn, helperPath = path.join(__dirname, '../native/voice/build/hibi-voice') } = {}) {
  let child = null;
  return {
    listen({ locale = 'pt-BR', onText } = {}) {
      child?.kill('SIGTERM');
      child = spawnProcess(helperPath, ['listen', locale], { stdio: ['ignore', 'pipe', 'ignore'] });
