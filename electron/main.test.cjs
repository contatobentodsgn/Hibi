  ...
  return {
    ...
    async get(account) {
      ...
      if (typeof secret !== "string" || !secret)
        throw new Error("No API key is stored for this provider.");
      return secret;
    },
...
async function loadMain({ seedUserData } = {}) {
  ...
  class BrowserWindowFake {
    constructor(options) {
      ...
      this.webContents = {
        on: (event, listener) => {
          owner.contentListeners.set(event, listener);
        },
        send: (...args) => {
        ...
        setWindowOpenHandler: (handler) => {
          owner.windowOpenHandler = handler;
        },
        reloadIgnoringCache: () => {
          owner.reloads += 1;
        },
  ...
  const electronFake = {
    app: {
      ...
      setLoginItemSettings: (value) => {
        loginItem = { ...loginItem, ...value };
      },
      on: (event, listener) => {
        appEvents.set(event, listener);
      },
      quit: () => {
        captured.quitCalls = (captured.quitCalls ?? 0) + 1;
      },
      whenReady: () => ({
        then: (callback) => {
    ...
    Notification: class NotificationFake {
      ...
      show() {
        captured.shown = (captured.shown ?? 0) + 1;
      }
    },
    screen: {
      getAllDisplays: () =>
      ...
      on: (event, listener) => {
        screenEvents.push([event, listener]);
      },
      removeListener: (event, listener) => {
        removedEvents.push([event, listener]);
      },
    },
    shell: { openExternal: () => Promise.resolve() },
    powerMonitor: {
      on: (event, listener) => {
        powerEvents.push([event, listener]);
      },
      removeListener: (event, listener) => {
        removedEvents.push([event, listener]);
      },
  ...
  const notchManager = {
    ...
    show(presentation) {
      this.calls.push(["show", presentation]);
    ...
    hide(requestId) {
      this.calls.push(["hide", requestId]);
      return requestId === this.shown?.requestId;
    },
    resolveAction(requestId, actionId) {
      this.calls.push(["resolveAction", requestId, actionId]);
      return true;
    },
    setPreferredDisplay(displayId) {
      this.calls.push(["setPreferredDisplay", displayId]);
      this.preferredDisplay = displayId;
    },
    reposition() {
      this.calls.push(["reposition"]);
      return true;
    },
    describeDisplays() {
      return {
  ...
  const localApi = {
    ...
    async start() {
      ...
      this.running = true;
      return { origin: "http://127.0.0.1:41234", token: "secret-token" };
    },
    async stop() {
  ...
  const webhookService = {
    ...
    async configure(secret) {
      this.calls.push(["configure", secret]);
    },
    async start() {
      this.calls.push(["start"]);
    },
  ...
  const oauthService = {
    calls: [],
    supports(id) {
      this.calls.push(["supports", id]);
      return id === "notion";
    },
    async authorize(id, options) {
      this.calls.push(["authorize", id, options]);
      return { ok: true };
    },
    async refresh(id, options) {
      this.calls.push(["refresh", id, options]);
      return { ok: true };
    },
    async cancel() {
  ...
  const notchTest = {
    ...
    run(locale) {
      ...
      return { outcome: "confirmed", displayId: 1, displayLabel: "Built-in" };
    },
    handleAction(action) {
      return (
        typeof action?.requestId === "string" &&
        action.requestId.startsWith(realNotchTest.NOTCH_TEST_PREFIX)
      );
    },
  };
  ...
  const stubs = {
    ...
    "./local-api.cjs": {
      createLocalApiTokenStore: () => ({
        getOrCreate: async () => "token",
        revoke: async () => true,
      }),
      createLocalApi: (options) => {
        captured.localApi = options;
        return localApi;
      },
    },
    "./webhooks.cjs": { createWebhookService: () => webhookService },
    "./oauth.cjs": {
      createOAuthService: (options) => {
        captured.oauth = options;
        return oauthService;
      },
    },
    "./notifications.mjs": {
      ...
      createNotificationScheduler: (options) => {
        ...
        const scheduler =
          realNotifications.createNotificationScheduler(options);
        ...
        scheduler.sync = (entries, context) => {
          captured.syncCalls.push({ entries, context });
          return sync(entries, context);
        };
        return scheduler;
      },
    },
    ...
    "./notch-window.cjs": {
      ...realNotchWindow,
      createNotchWindowManager: (options) => {
        captured.notchWindow = options;
        return notchManager;
      },
    },
    "./notch-test.cjs": {
      ...realNotchTest,
      createNotchTest: (options) => {
        captured.notchTest = options;
        return notchTest;
      },
    },
    "../native/notch/index.cjs": {
      createNotchAdapter: (options) => {
        captured.adapter = options;
        return notchAdapter;
      },
    },
  };
  ...
  for (const key of Object.keys(require.cache)) {
    if (
      key !== __filename &&
      (key.startsWith(`${__dirname}${path.sep}`) ||
        key.includes(`${path.sep}native${path.sep}notch${path.sep}`))
    )
  ...
  Module._load = function (request, parent, isMain) {
    ...
    if (parent?.filename === MAIN_PATH && Object.hasOwn(stubs, request))
  ...
  return {
    ...
    setNotificationSupported: (value) => {
      notificationSupported = value;
    },
    setSystemIdleSeconds: (value) => {
      systemIdleSeconds = value;
    },
    ...
    firePower: (name) => {
      ...
      for (const [event, listener] of powerEvents)
        if (event === name && !removed.has(listener)) listener();
    },
    invoke: (channel, ...args) => {
...
test("todo canal que o processo principal emite é escutado pelo preload", async (t) => {
  ...
  const listened = new Set(
    [...preload.matchAll(/ipcRenderer\.on\(\s*['"]([^'"]+)['"]/g)].map(
      (match) => match[1],
    ),
  );
  const emitted = new Set(
    [MAIN_PATH, NOTCH_WINDOW_PATH].flatMap((file) =>
      [
        ...fs
          .readFileSync(file, "utf8")
          .matchAll(/(?:sendToMainWindow|send)\(\s*['"](hibi:[^'"]+)['"]/g),
      ].map((match) => match[1]),
    ),
  );
...
test("a janela principal nasce isolada do Node e com a política de navegação ligada", async (t) => {
  ...
  willNavigate(
    { preventDefault: () => prevented.push("https://evil.example") },
    "https://evil.example",
  );
  willNavigate(
    { preventDefault: () => prevented.push("http://127.0.0.1:5173/") },
    "http://127.0.0.1:5173/",
  );
  assert.deepEqual(prevented, ["https://evil.example"]);

  assert.deepEqual(window.windowOpenHandler({ url: "https://evil.example" }), {
    action: "deny",
  });
  assert.deepEqual(
    window.windowOpenHandler({
      url: "file:///Applications/Hibi.app/index.html",
    }),
    { action: "allow" },
  );
});
...
test("hibi:info devolve a versão do app e não vaza caminhos locais", async (t) => {
  ...
  assert.deepEqual(await harness.invoke("hibi:info"), {
...
test("hibi:login-item converte qualquer payload do renderer em booleano", async (t) => {
  ...
  assert.equal(
    await harness.invoke("hibi:login-item", { openAtLogin: true }),
    true,
  );
});
...
test("hibi:notifications:sync descarta entradas inválidas em vez de falhar", async (t) => {
  ...
  await harness.invoke("hibi:notifications:sync", [
    { id: "", title: "", body: "", kind: "outro", at: "ontem" },
    null,
    42,
  ]);
...
test("hibi:ai:run recusa um correlationId fora do formato", async (t) => {
  ...
  for (const correlationId of [
    ...
    await assert.rejects(
      async () => harness.invoke("hibi:ai:run", { correlationId, request }),
      /Invalid AI correlation id/,
    );
  }
});
...
test("hibi:ai:run devolve o turno correlacionado e transmite só eventos saneados", async (t) => {
  ...
  const result = await harness.invoke("hibi:ai:run", {
    correlationId: "renderer-1",
    request: { message: "  oi  ", surface: "desktop" },
  });
  ...
  const streamed = harness.event.sender.sent.filter(
    ([channel]) => channel === "hibi:ai:stream",
  );
...
test("hibi:ai:run não escreve num renderer já destruído", async (t) => {
  ...
  await harness.invoke("hibi:ai:run", {
    correlationId: "renderer-2",
    request: { message: "oi", surface: "desktop" },
  });
...
test("hibi:ai:run recusa um turno malformado antes de chegar ao provedor", async (t) => {
  ...
  await assert.rejects(
    async () =>
      harness.invoke("hibi:ai:run", {
        correlationId: "renderer-3",
        request: { message: "oi", surface: "browser" },
      }),
    /Invalid AI surface/,
  );
  await assert.rejects(
    async () =>
      harness.invoke("hibi:ai:run", {
        correlationId: "renderer-3",
        request: { message: "   ", surface: "desktop" },
      }),
    /Invalid AI message/,
  );
});
...
test("hibi:ai:cancel ignora um payload vazio em vez de cancelar o turno de outra pessoa", async (t) => {
  ...
  assert.equal(
    await harness.invoke("hibi:ai:cancel", { requestId: "ai-inexistente" }),
    false,
  );
});
...
test("hibi:ai-config:save recusa uma configuração inválida e mantém o runtime anterior", async (t) => {
  ...
  await assert.rejects(
    async () => harness.invoke("hibi:ai-config:save", { provider: "qualquer" }),
    /Invalid AI provider/,
  );
  await assert.rejects(
    async () =>
      harness.invoke("hibi:ai-config:save", {
        provider: "openai-compatible",
        endpoint: "http://provedor.example/v1",
        model: "m",
      }),
    /HTTPS or local loopback/,
  );
  assert.deepEqual(await harness.invoke("hibi:ai-config:get"), {
    provider: "local",
    endpoint: "",
...
test("trocar o endpoint de um conector preserva a auditoria e descarta as ações já preparadas", async (t) => {
  ...
  const prepared = await harness.invoke("hibi:integrations:prepare-action", {
    connectorId: "remote-notifications",
    kind: "notification.send",
    payload: { title: "Estudar", body: "Bloco de estudo" },
  });
  ...
  await harness.invoke(
    "hibi:integrations:save-settings",
    "remote-notifications",
    { endpoint: "https://notifications.example.test/v2" },
  );
  ...
  await assert.rejects(
    async () =>
      harness.invoke("hibi:integrations:execute-approved", {
...
test("hibi:integrations:save-settings recusa um endpoint inseguro", async (t) => {
  ...
  await assert.rejects(
    async () =>
      harness.invoke("hibi:integrations:save-settings", "notion", {
        endpoint: "http://notion.example.test",
      }),
    /must use HTTPS/,
  );
  await assert.rejects(
    async () =>
      harness.invoke("hibi:integrations:save-settings", "notion", {
        endpoint: "https://user:pass@notion.example.test",
      }),
    /must not embed credentials/,
  );
  await assert.rejects(
    async () => harness.invoke("hibi:integrations:get-settings", "../etc"),
    /Unknown integration connector/,
  );
});
...
test("hibi:integrations:connect recusa uma credencial vazia e não grava nada no Keychain", async (t) => {
  ...
  await assert.rejects(
    async () => harness.invoke("hibi:integrations:connect", "notion", ""),
...
test("os handlers de OAuth levam o clientId configurado do conector", async (t) => {
  ...
  await harness.invoke("hibi:integrations:save-settings", "notion", {
    clientId: "cliente-1",
  });
...
test("hibi:local-api:sync-workspace normaliza e limita o que o renderer envia", async (t) => {
  ...
  assert.deepEqual(harness.captured.localApi.workspace(), {
    tasks: [],
    reminders: [],
    blocks: [],
  });

  await harness.invoke("hibi:local-api:sync-workspace", {
    tasks: "muitas",
    reminders: [{ id: "r" }],
    blocks: null,
  });
  ...
  await harness.invoke("hibi:local-api:sync-workspace", {
    tasks: Array.from({ length: 6_000 }, (_, index) => index),
  });
...
test("uma escrita da API local só pode ser resolvida uma vez", async (t) => {
  ...
  const prepared = await harness.captured.localApi.prepareWrite({
    kind: "task.create",
    payload: { title: "Estudar" },
  });
  assert.equal(prepared.requiresConfirmation, true);
  assert.match(prepared.confirmationId, /^local-api-[0-9a-f-]{36}$/);
  assert.deepEqual(harness.mainWindow().sent.at(-1), [
    "hibi:local-api:confirmation",
    {
      confirmationId: prepared.confirmationId,
      kind: "task.create",
      payload: { title: "Estudar" },
  ...
  assert.deepEqual(
    await harness.invoke("hibi:local-api:resolve-write", {
      confirmationId: prepared.confirmationId,
      approved: true,
    }),
...
test("hibi:local-api:resolve-write ignora confirmações desconhecidas ou malformadas", async (t) => {
  ...
  assert.deepEqual(
    await harness.invoke("hibi:local-api:resolve-write", {
      confirmationId: "local-api-inventado",
    }),
    { resolved: false },
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    await harness.invoke("hibi:local-api:resolve-write", undefined),
    { resolved: false },
  );
...
test("os handlers da API local e do webhook devolvem só o resumo, sem o token", async (t) => {
  ...
  assert.deepEqual(await harness.invoke("hibi:local-api:start"), {
    origin: "http://127.0.0.1:41234",
  });
  assert.deepEqual(await harness.invoke("hibi:local-api:status"), {
    running: true,
  });
  assert.deepEqual(await harness.invoke("hibi:local-api:stop"), {
    running: false,
  });
  assert.deepEqual(await harness.invoke("hibi:webhook:configure", "segredo"), {
    running: true,
    configured: true,
  });
  assert.deepEqual(harness.webhookService.calls.at(-1), [
    "configure",
    "segredo",
  ]);
});
...
test("hibi:notch:show recusa apresentações inválidas e as do prefixo reservado ao teste", async (t) => {
  ...
  await assert.rejects(
    async () => harness.invoke("hibi:notch:show", null),
    /Invalid companion presentation/,
  );
  await assert.rejects(
    async () =>
      harness.invoke("hibi:notch:show", {
        requestId: "c-1",
        kind: "confirmation",
        text: "Ok?",
        actions: [1, 2, 3, 4, 5],
  ...
  await assert.rejects(
    async () =>
      harness.invoke("hibi:notch:show", {
        requestId: `${realNotchTest.NOTCH_TEST_PREFIX}confirm-1`,
        kind: "confirmation",
        text: "Ok?",
        actions: [],
      }),
  ...
  assert.deepEqual(await harness.invoke("hibi:notch:show", presentation), {
    degraded: true,
    requestId: "c-1",
    host: "electron",
  });
...
test("hibi:notch:action e hibi:notch:hide só aceitam identificadores limitados", async (t) => {
  ...
  assert.equal(
    await harness.invoke("hibi:notch:action", "c-1", "apagar"),
    false,
  );
  ...
  assert.deepEqual(harness.notchManager.calls.at(-1), [
    "resolveAction",
    "c-1",
    "confirm",
  ]);
...
test("as respostas do teste do notch não chegam ao renderer", async (t) => {
  ...
  harness.captured.notchWindow.onAction({
    requestId: `${realNotchTest.NOTCH_TEST_PREFIX}confirm-1`,
    actionId: "confirm",
  });
  ...
  assert.deepEqual(harness.mainWindow().sent, [
    ["hibi:companion:action", { requestId: "c-1", actionId: "cancel" }],
  ]);
});
...
test("hibi:notch:set-display recusa um monitor inexistente e persiste o escolhido", async (t) => {
  ...
  await assert.rejects(
    async () => harness.invoke("hibi:notch:set-display", 99),
    /Invalid notch display/,
  );
  await assert.rejects(
    async () => harness.invoke("hibi:notch:set-display", "7"),
    /Invalid notch display/,
  );
  ...
  assert.deepEqual(
    JSON.parse(
      fs.readFileSync(
        path.join(harness.userData, "notch-settings.json"),
        "utf8",
      ),
    ),
...
test("a preferência de monitor salva chega ao gerenciador na inicialização", async (t) => {
  const harness = await loadMain({
    seedUserData: (userData) =>
      fs.writeFileSync(
        path.join(userData, "notch-settings.json"),
        JSON.stringify({ displayId: 7, displayLabel: "Studio Display" }),
      ),
  });
  t.after(() => harness.cleanup());
...
test("uma mudança de monitores reposiciona a companion e avisa a janela principal", async (t) => {
  ...
  const [, listener] = harness.screenEvents.find(
    ([name]) => name === "display-added",
  );
  listener();

  assert.deepEqual(harness.notchManager.calls.at(-1), ["reposition"]);
  assert.deepEqual(harness.mainWindow().sent.at(-1), [
    "hibi:notch:displays-changed",
  ]);
});
...
test("before-quit encerra serviços, solta os listeners e destrói a companion", async (t) => {
  ...
  assert.deepEqual(
    harness.removedEvents.map(([event]) => event),
    ["display-added", "display-removed", "display-metrics-changed", "resume"],
  );
});
...
test("hibi:notifications:sync entrega ao agendador os ajustes e a janela de foco do renderer", async (t) => {
  ...
  const context = {
    ...
    focusUntilMs: 1_900_000_000_000,
  };
...
function loadPreload() {
  ...
  const originalLoad = Module._load;
  Module._load = function (request, parent, isMain) {
    if (request === "electron" && parent?.filename === PRELOAD_PATH)
      return electronStub;
    return originalLoad.call(this, request, parent, isMain);
  };
  try {
...
const presenceSent = (harness) =>
  harness
    .mainWindow()
    .sent.filter(([channel]) => channel === PRESENCE_CHANNEL)
    .map(([, event]) => event);
const POWER_PRESENCE_EVENTS = [
  "lock-screen",
  "suspend",
  "unlock-screen",
  "resume",
...
test("presença: sem sessão de foco não há polling nem ouvinte de bloqueio e sono", async (t) => {
  ...
  assert.deepEqual(
    harness.powerEvents.map(([event]) => event),
    ["resume"],
  );
  ...
  await harness.invoke("hibi:focus:watch-presence", {
    watching: "true",
    idleMinutes: 5,
  });
  ...
  assert.equal(harness.idleQueries(), 2);
  assert.deepEqual(
    harness.removedEvents.map(([event]) => event).sort(),
    [...POWER_PRESENCE_EVENTS].sort(),
  );
...
test("presença: inatividade acima do limiar durante a sessão emite ausência, e o retorno emite retorno", async (t) => {
  ...
  assert.deepEqual(
    presenceSent(harness),
    [],
    "abaixo do limiar não há ausência",
  );
  ...
  assert.deepEqual(presenceSent(harness), [
    { type: "away", reason: "idle", idleSeconds: 300, atMs: awayAtMs },
  ]);
  ...
  t.mock.timers.tick(PRESENCE_POLL_MS);
  assert.deepEqual(presenceSent(harness).at(-1), {
    type: "returned",
    reason: "idle",
    awaySeconds: 300 + 20,
    atMs: awayAtMs + PRESENCE_POLL_MS * 2,
  });
});
...
test("presença: bloquear a tela e dormir contam como ausência por energia, e a volta só vem com a tela desbloqueada", async (t) => {
  ...
  harness.firePower("lock-screen");
  assert.deepEqual(presenceSent(harness), [
    { type: "away", reason: "power", idleSeconds: 40, atMs: PRESENCE_START_MS },
  ]);
  ...
  assert.equal(
    presenceSent(harness).length,
    1,
    "acordou, mas ainda está na tela de senha",
  );
  ...
  assert.deepEqual(presenceSent(harness).at(-1), {
    type: "returned",
    reason: "power",
    awaySeconds: 40 + 3_600,
    atMs: PRESENCE_START_MS + 3_600_000,
  });
});
...
test("o evento de presença atravessa a ponte: processo principal → preload → renderer, e a vigia volta", async (t) => {
  ...
  const received = [];
  const unsubscribe = preload.api.onFocusPresence((event) =>
    received.push(event),
  );
  ...
  const emitted = harness
    .mainWindow()
    .sent.filter(([sentChannel]) => sentChannel === PRESENCE_CHANNEL);
  assert.equal(emitted.length, 1);
  for (const [sentChannel, ...args] of emitted)
    preload.deliver(sentChannel, ...args);
  assert.deepEqual(received, [
    {
      type: "away",
      reason: "idle",
      idleSeconds: 60,
      atMs: PRESENCE_START_MS + PRESENCE_POLL_MS,
    },
  ]);
