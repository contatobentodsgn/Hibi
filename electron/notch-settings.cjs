...
const MAX_LABEL = 120;
const defaults = () => ({ displayId: null, displayLabel: '', size: 'normal' });

function normalizeNotchSettings(value) {
  if (!value || typeof value !== 'object') throw new Error('Invalid notch settings.');
  if (value.displayId === null) return { ...defaults(), size: value.size === 'compact' ? 'compact' : 'normal' };
  ...
  return { displayId: value.displayId, displayLabel: (value.displayLabel ?? '').trim().slice(0, MAX_LABEL), size: value.size === 'compact' ? 'compact' : 'normal' };
}
...
function applyNotchDisplay(settings, manager, displayId) {
  if (displayId === null) {
    settings.save({ ...settings.get(), displayId: null });
  ...
  settings.save({ ...settings.get(), displayId: target.id, displayLabel: target.label });
