// Compila o ícone do app (build/icon/Pixano.icon, formato do Icon Composer, com as versões clara e escura)
...
try {
  execFileSync('xcrun', ['actool', join(root, 'build/icon/Pixano.icon'), '--compile', out, '--app-icon', 'Pixano', '--platform', 'macosx', '--minimum-deployment-target', '12.0', '--output-partial-info-plist', join(out, 'partial.plist'), '--errors', '--warnings'], { env, stdio: 'inherit' });
  copyFileSync(join(out, 'Assets.car'), join(root, 'build/icon/Assets.car'));
  copyFileSync(join(out, 'Pixano.icns'), join(root, 'build/icon/icon.icns'));
