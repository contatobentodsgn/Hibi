...
test('o pacote do mac leva o ícone do Pixano, nas duas formas, com o nome que o catálogo usa', () => {
  ...
  assert.equal(mac.extendInfo.CFBundleIconName, 'Pixano');
});

test('o ícone de origem tem as duas aparências, e a barra de menus usa imagem-modelo', () => {
  const icon = JSON.parse(readFileSync(join(root, 'build/icon/Pixano.icon/icon.json'), 'utf8'));
  ...
  for (const layer of layers) assert.ok(existsSync(join(root, 'build/icon/Pixano.icon/Assets', layer['image-name'])));
