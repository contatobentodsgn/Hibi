...
export function readLiveConnectorConfig(environment = process.env) {
  if (environment.PIXANO_LIVE_CONNECTOR_TEST !== '1') throw new Error('Set PIXANO_LIVE_CONNECTOR_TEST=1 to permit a real connector test.')
  const connectorId = environment.PIXANO_LIVE_CONNECTOR_ID
  const endpoint = environment.PIXANO_LIVE_CONNECTOR_ENDPOINT
  const token = environment.PIXANO_LIVE_CONNECTOR_TOKEN
  const allowed = (environment.PIXANO_LIVE_CONNECTOR_ALLOW_HOSTS ?? '').split(',').map((value) => value.trim().toLowerCase()).filter(Boolean)
  if (!Object.hasOwn(FACTORIES, String(connectorId))) throw new Error(`Live connector test requires PIXANO_LIVE_CONNECTOR_ID to be one of: ${Object.keys(FACTORIES).join(', ')}.`)
  ...
  const targets = (environment.PIXANO_LIVE_CONNECTOR_TARGETS ?? '').split(',').map((value) => value.trim()).filter(Boolean).map((id) => ({ id }))
  ...
  const lifecycleEnabled = environment.PIXANO_LIVE_NOTION_LIFECYCLE === '1'
  if (environment.PIXANO_LIVE_CONNECTOR_WRITE_TEST === '1' && !(lifecycleEnabled && environment.PIXANO_LIVE_CONNECTOR_WRITE_KIND === undefined)) {
    const kind = environment.PIXANO_LIVE_CONNECTOR_WRITE_KIND
    if (!bounded(kind, 120)) throw new Error('A real write requires PIXANO_LIVE_CONNECTOR_WRITE_KIND.')
    let payload
    try { payload = JSON.parse(environment.PIXANO_LIVE_CONNECTOR_WRITE_PAYLOAD ?? '') } catch { throw new Error('A real write requires PIXANO_LIVE_CONNECTOR_WRITE_PAYLOAD as JSON.') }
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) throw new Error('A real write requires PIXANO_LIVE_CONNECTOR_WRITE_PAYLOAD as a JSON object.')
    write = { kind, payload }
  }
...
export function readNotionLifecycleConfig(environment = process.env) {
  if (environment.PIXANO_LIVE_NOTION_LIFECYCLE !== '1') return null
  if (environment.PIXANO_LIVE_CONNECTOR_ID !== 'notion') throw new Error('The Notion lifecycle requires PIXANO_LIVE_CONNECTOR_ID=notion.')
  if (environment.PIXANO_LIVE_CONNECTOR_WRITE_TEST !== '1') throw new Error('The Notion lifecycle writes to the workspace: set PIXANO_LIVE_CONNECTOR_WRITE_TEST=1 as a separate opt-in.')
  const dataSourceId = environment.PIXANO_LIVE_NOTION_DATA_SOURCE
  if (!bounded(dataSourceId, 240)) throw new Error('The Notion lifecycle requires PIXANO_LIVE_NOTION_DATA_SOURCE.')
  return { dataSourceId }
}
...
export async function runLiveConnectorTest(environment = process.env, fetch = globalThis.fetch) {
  ...
  const manager = createIntegrationManager({ connectors: [connector], keychain: inMemoryKeychain(safe.connectorId, environment.PIXANO_LIVE_CONNECTOR_TOKEN), fetch })
