...
test('refuses live provider traffic without explicit opt-in and a host allowlist', () => {
  assert.throws(() => readLiveProviderConfig({ PIXANO_LIVE_PROVIDER_ENDPOINT: 'https://sandbox.example.test/v1/chat/completions', PIXANO_LIVE_PROVIDER_MODEL: 'test', PIXANO_LIVE_PROVIDER_KEY: 'secret', PIXANO_LIVE_PROVIDER_ALLOW_HOSTS: 'sandbox.example.test' }), /PIXANO_LIVE_PROVIDER_TEST=1/)
  assert.throws(() => readLiveProviderConfig({ PIXANO_LIVE_PROVIDER_TEST: '1', PIXANO_LIVE_PROVIDER_ENDPOINT: 'https://outside.example.test/v1/chat/completions', PIXANO_LIVE_PROVIDER_MODEL: 'test', PIXANO_LIVE_PROVIDER_KEY: 'secret', PIXANO_LIVE_PROVIDER_ALLOW_HOSTS: 'sandbox.example.test' }), /allowlist/)
})

test('accepts an explicitly opted-in sandbox configuration without returning the key', () => {
  const config = readLiveProviderConfig({ PIXANO_LIVE_PROVIDER_TEST: '1', PIXANO_LIVE_PROVIDER_ENDPOINT: 'https://sandbox.example.test/v1/chat/completions', PIXANO_LIVE_PROVIDER_MODEL: 'test', PIXANO_LIVE_PROVIDER_KEY: 'secret', PIXANO_LIVE_PROVIDER_ALLOW_HOSTS: 'sandbox.example.test' })
...
const sandboxEnvironment = {
  PIXANO_LIVE_PROVIDER_TEST: '1',
  PIXANO_LIVE_PROVIDER_ENDPOINT: 'https://sandbox.example.test/v1/chat/completions',
  PIXANO_LIVE_PROVIDER_MODEL: 'test-model',
  PIXANO_LIVE_PROVIDER_KEY: 'secret',
  PIXANO_LIVE_PROVIDER_ALLOW_HOSTS: 'sandbox.example.test',
}
