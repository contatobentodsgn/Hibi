...
| --- | --- | --- |
| Persistence | `StudyData` is owned by one in-memory `LocalRepository`. In Electron, the renderer hydrates and persists it through SQLite in the app's user-data directory; writes are transactional and create restore points before replacement/restoration. A first launch migrates `hibi-study-data` from `localStorage` once. Browser-only mode keeps the JSON `localStorage` fallback. UI events use `hibi-events`. | No server, account, sync, or encrypted-store contract is present. |
...
| Calendar | Calendar surfaces read/write local `ScheduleBlock` records (`start`, `end`, category, optional task link, hard flag). The Electron sync service also has Apple EventKit and Google Calendar adapters for discovery, read/import, prepared writes, confirmation-gated publishing, and conflict handling. Google OAuth uses PKCE, a loopback callback, Keychain-held access/refresh tokens, and an optional Keychain-held client secret; successful authorization discovers all Google calendars automatically. | Local blocks and ICS work without an account. Apple access requires macOS permission; real Google authorization still requires the app's OAuth client ID/secret and consent in Google Cloud. |
...
- `App` hydrates from the Electron SQLite bridge when available, migrates the legacy `localStorage` JSON once when the database is empty, and persists each repository snapshot after state changes. Browser-only mode restores/saves the legacy JSON ([App.tsx](../src/App.tsx), [workspace-database.cjs](../electron/workspace-database.cjs)).
...
The current source does not establish contracts for cloud persistence, account sync, remote notifications, microphone/camera capture, notch hardware control, or executing extracted native/runtime assets. External calendar contracts do exist locally for Apple EventKit and Google Calendar, but account authorization and real-service validation remain platform-dependent; Google token exchange still requires client-secret support in the secure main-process flow. Treat UI labels or historical parity notes about those areas as availability statements, not implemented interfaces ([parity-audit.md](./parity-audit.md), [hey-taby-replica-design.md](./hey-taby-replica-design.md)). The weekly calendar also provides local ICS import/export through browser file/blob APIs.
