...
- **Arrastar a janela (macOS).** O Chromium monta as regiões de arrastar na ordem da árvore, uma por caixa marcada, e a mais recente vence; o valor é herdado, e até `none` vira `no-drag`. Por isso nada acima da barra declara `-webkit-app-region`, os notches marcam `no-drag` e o conteúdo não declara região nenhuma. Desde a revisão da U03, a barra vem antes do conteúdo na árvore, como no preview, para o Tab chegar a ela primeiro; com o conteúdo sem região, a ordem não muda as regiões. O que rola por baixo da faixa fica sob uma borda da cor da superfície (`.notch-scroll-edge`), porque ali o clique é da janela. Um e2e reproduz o algoritmo, e o app real foi arrastado de verdade (pela faixa move; pelo conteúdo e pelos botões, não).
