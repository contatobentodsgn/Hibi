...
| --- | --- | --- |
| Built-in display with camera housing | Trigger a task completion, reminder, focus start, validation error and AI confirmation. | Passive cards do not take focus or intercept clicks. Confirmation is centered below the physical housing, takes keyboard focus, and Confirm/Cancel both work. |
...
| External display | Attach a display, make it primary, then trigger passive and confirmation cards. | If the built-in Mac screen has a housing, the companion remains on that physical display. Otherwise it follows the selected primary display. |
...
| VoiceOver and keyboard | Enable VoiceOver, trigger a confirmation, then use Tab, Shift-Tab, Enter and Escape. | VoiceOver announces “Hibi confirmation” and its message. The first action is focused, Tab reaches all actions, Enter activates the focused action, and Escape dismisses passive cards. |
