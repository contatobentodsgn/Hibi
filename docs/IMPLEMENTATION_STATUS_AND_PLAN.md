...
- **O OAuth configurável ainda não está ligado ao `main.cjs`.** A exclusão entre OAuth e endpoint customizado foi resolvida por design: as URLs de autorização passaram a ser configuráveis por conector (`authorizationUrl` e `tokenUrl` em `connector-settings.cjs`, com a mesma barreira do endpoint — HTTPS, sem credencial embutida, sem query nem fragmento), e `electron/connectors/index.cjs` monta os conectores a partir delas. Sem as duas URLs, um endpoint próprio passa a **não ter OAuth** em vez de falhar com "not allowed for this connector": `supports()` devolve `false` e a tela diz que o conector usa credencial direta. O OAuth embutido de Notion e Slack continua valendo no endpoint padrão; o do e-mail foi removido por apontar para `mail.example.test`, que não existe. **O que falta:** `electron/main.cjs` ainda tem a própria `buildConnectors` local — enquanto ela não for trocada por `require('./connectors/index.cjs')`, o conserto não chega ao app empacotado. É uma troca de duas linhas, deixada de fora por o arquivo estar sendo alterado em paralelo. *(auditoria de 2026-09-11 · corrigido e testado em 2026-09-11)*
