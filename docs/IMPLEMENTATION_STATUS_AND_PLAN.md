...
- **Falta liberar `invalid_request` no `main.cjs`.** A classificação foi corrigida: nos dois `classifyProviderFailure` (renderer e processo principal) todo 4xx não mapeado virou `invalid_request` não-retentável, `invalid_response` ficou reservado à resposta que de facto não se lê, e os Ajustes de IA passaram a nomear o endpoint e o modelo em vez de culpar a resposta. Falta uma linha, num arquivo que estava sendo refatorado por outra tarefa: o allowlist de `safeAiFailure` em `electron/main.cjs:92` ainda lista só os cinco códigos antigos, então um evento `failed` com o código novo é descartado a caminho do renderer e o turno falha sem cartão nenhum. Incluir `'invalid_request'` nessa lista encerra o item. *(2026-09-11)*
