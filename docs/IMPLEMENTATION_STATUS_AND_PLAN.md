...
- **Uma postagem recusada pelo Slack vira escrita bem-sucedida.** `executeApproved` em `electron/connectors/slack.cjs` devolve a resposta HTTP sem verificar o `ok` do corpo, e a API do Slack recusa chamadas com HTTP 200; a ação entra na auditoria como executada sem nada ter sido postado. *(reproduzido contra a sandbox em 2026-09-11 · tarefa criada)*
