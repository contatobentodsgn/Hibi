...
async function installBridge(page: Page, status: 'active' | 'taken' = 'active') {
  await page.addInitScript((initial) => {
    ...
    (window as unknown as { hibiDesktop: unknown }).hibiDesktop = {
      getTabyShortcut: async () => state,
      setTabyShortcut: async (accelerator: string | null) => { recorded.push(accelerator); state = { accelerator, status: accelerator ? 'active' : 'disabled' }; return state; },
      onTabyShortcut: (callback: () => void) => { listeners.push(callback); return () => listeners.splice(listeners.indexOf(callback), 1); },
    };
  }, status);
}

test('o atalho global abre o Taby de qualquer tela', async ({ page }) => {
...
test('a pessoa escolhe a combinação em Ajustes, e o pedido chega ao sistema', async ({ page }) => {
  ...
  expect(await page.evaluate(() => (window as unknown as { hibiE2E: { recorded: (string | null)[] } }).hibiE2E.recorded)).toEqual(['Option+Space', null]);
});
