...
test('a no-match notes state explains itself and restores the list', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('navigation', { name: 'Navegação principal' }).getByRole('button', { name: 'Mais seções', exact: true }).click();
  await page.getByRole('menuitem', { name: 'Notas', exact: true }).click();
