...
const seedOverlap = async (page: Page, day: string) => {
  await page.evaluate((date) => {
    ...
    data.blocks = [...(data.blocks ?? []), { id: 'e2e-aula', title: 'Aula', start: `${date}T14:00:00`, end: `${date}T16:00:00`, category: 'learning' }, { id: 'e2e-dentista', title: 'Dentista', start: `${date}T15:00:00`, end: `${date}T15:30:00`, category: 'work' }];
...
test('a semana mostra os blocos que se sobrepõem, não um "sem conflitos" fixo', async ({ page }) => {
  await agenda(page, 'Semana');
  await expect(page.getByRole('status').filter({ hasText: 'Nenhum bloco se sobrepõe nesta semana.' })).toBeVisible();

  await seedOverlap(page, '2026-09-08');
  await agenda(page, 'Semana');

  const aviso = page.getByRole('status').filter({ hasText: 'Blocos sobrepostos:' });
  await expect(aviso).toContainText('Aula (08/09 14:00) × Dentista (08/09 15:00)');
  await expect(page.getByText('Nenhum bloco se sobrepõe nesta semana.')).toHaveCount(0);
});
...
test('no Dia, "Conferir plano" confere de verdade e "24h" mostra a madrugada', async ({ page }) => {
  ...
  await expect(page.getByText('Blocos sobrepostos:')).toHaveCount(0);
  await page.getByRole('button', { name: '✓ Conferir plano' }).click();
  await expect(page.getByRole('status').filter({ hasText: 'Blocos sobrepostos:' })).toContainText('Aula (07/09 14:00) × Dentista (07/09 15:00)');
});
