...
async function installBridge(page: Page, initial: string) {
  await page.addInitScript((status) => {
    ...
    (window as unknown as { pixanoE2E: unknown }).pixanoE2E = { recorded, publish };
...
test('procurar, baixar e instalar são três pedidos, um de cada vez', async ({ page }) => {
  ...
  await page.evaluate(() => (window as unknown as { pixanoE2E: { publish: (patch: Record<string, unknown>) => void } }).pixanoE2E.publish({ status: 'available', version: '0.2.0' }));
  ...
  expect(await page.evaluate(() => (window as unknown as { pixanoE2E: { recorded: string[] } }).pixanoE2E.recorded)).toEqual(['check', 'download', 'install']);
});
...
test('uma falha aparece na tela com o motivo, e não some em silêncio', async ({ page }) => {
  ...
  await page.evaluate(() => (window as unknown as { pixanoE2E: { publish: (patch: Record<string, unknown>) => void } }).pixanoE2E.publish({ status: 'error', error: 'Could not get code signature for running application' }));
